...
local _TopHpPatePrefab, _fightEnergyPatePrefab, _PlayerTopPatePrefab, _MinePatePrefab, _FightSelectPrefab, _FightCmdPrefab, _FightBuffPrefab, _TalkPrefab, _TopButton, _BallCooldownPrefab
...
def.static("function").LoadPatePrefab = function(onFinish)
  AsyncLoadArray({
    ...
    _fightEnergyPatePrefab = assetArr[10]
    if onFinish then
      onFinish()
    end
  end)
...
def.method("userdata", "userdata", "userdata", "number", "=>", "userdata")._CreateFromCacheByHp = function(self, obj, cache, prefab, offsetH)
  ...
  if cache and cache.childCount > 0 then
    ...
    hud.parent = ECGUIMan.Instance():GetHudTopBoardRootWithDepth()
...
def.method(FightModel).CreateEnergyBoard = function(self, fightmodel)
  local function doCreate()
    local attachObj = fightmodel.m_model
    local offsetH = fightmodel:GetBoxHeight()
    if offsetH == 0 then
      ...
      offsetH = offsetH * attachObj.localScale.x + 0.3
    end
    if attachObj == nil or attachObj:get_isnil() then
      return
    end
    local pate = self:_CreateFromCacheByHp(attachObj, nil, _fightEnergyPatePrefab, offsetH)
    pate.localScale = pate_scale
    pate.localPosition = topLayer_pos
    pate:SetLayer(ClientDef_Layer.PateTextDepth)
    pate:SetActive(true)
    local hud = self.m_pate.parent:GetComponent("HUDFollowTarget")
    hud.offsetScale = false
    fightmodel.m_energyPate = self.m_pate.parent
    fightmodel:SetEnergy(fightmodel.energy)
  end
  if _TopHpPatePrefab then
    doCreate()
  else
...
def.method("table", "function").CreateTopBoard = function(self, player, cb)
  local function doCreate()
    ...
    player:SetPate()
    if player:is(FightModel) then
      player:SetTitleIcon(player.statusTitleIcon)
    else
