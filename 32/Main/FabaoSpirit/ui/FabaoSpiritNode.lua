...
local ListShowType = {List = 1, Grid = 2}
def.field("number").showType = ListShowType.List
def.const("string").CFG_LIST_TYPE = "SpiritListType"
...
def.override().OnHide = function(self)
  ...
  self.showType = ListShowType.List
end
...
def.method().SetLeftList = function(self)
  self._uiGOs.groupLeft:FindDirect("Btn_ChangeModel/Img_List"):SetActive(self.showType == ListShowType.List)
  self._uiGOs.groupLeft:FindDirect("Btn_ChangeModel/Img_Grid"):SetActive(self.showType == ListShowType.Grid)
  self._uiGOs.groupLeft:FindDirect("Group_ListModel"):SetActive(self.showType == ListShowType.List)
  self._uiGOs.groupLeft:FindDirect("Group_GridModel"):SetActive(self.showType == ListShowType.Grid)
  if self.showType == ListShowType.List then
    self:UpdateLeftList()
  else
    self:UpdateLeftGrid()
  end
...
def.method().UpdateLeftList = function(self)
  ...
  local ctrlScrollView = self._uiGOs.groupLeft:FindDirect("Group_ListModel/Scroll View_List")
...
def.method().UpdateLeftGrid = function(self)
  local ownedLQNum = FabaoSpiritModule.CountOwnedLQ()
  if ownedLQNum == 0 then
    self:SwithToEmptyUI(true)
    return
  end
  local ctrlScrollView = self._uiGOs.groupLeft:FindDirect("Group_GridModel/ScrollView")
  local ctrlUIList = ctrlScrollView:FindDirect("List_Icon")
  local ctrlLQList = GUIUtils.InitUIList(ctrlUIList, ownedLQNum)
  local ownedLQList = self._arrOwnedLQ
  self._status = self._status or {}
  self._status.selectAttrIdx = 1
  ...
  for i = 1, #ctrlLQList do
    ...
    self:FillGridLQItemInfo(ctrlLQItm, LQInfo, i)
  end
  local comScrollView = ctrlScrollView:GetComponent("UIScrollView")
  if #ctrlLQList > 0 then
    do
...
def.method("userdata", "table", "number").FillGridLQItemInfo = function(self, ctrl, LQInfo, idx)
  local imgWear = ctrl:FindDirect("Img_Wear_" .. idx)
  local itemIcon = ctrl:FindDirect("Icon_Equip_" .. idx)
  local lblLv = ctrl:FindDirect("Group_Lv_" .. idx .. "/Label_LvName_" .. idx)
  local imgRedPt = ctrl:FindDirect("Img_Red_" .. idx)
  local bgFrame = ctrl:FindDirect("Icon_BgEquip_" .. idx)
  imgWear:SetActive(LQInfo.bEquiped)
  GUIUtils.SetText(lblLv, textRes.FabaoSpirit[1]:format(LQInfo.level))
  GUIUtils.SetTexture(itemIcon, LQInfo.icon)
  imgRedPt:SetActive(LQInfo.tgNew or FabaoSpiritModule.CanFabaoSpiritLevelUp(LQInfo.clsId))
  local itemId = FabaoSpiritUtils.GetItemIdByCfgId(LQInfo.LQItemId)
  GUIUtils.SetSprite(bgFrame, ItemUtils.GetItemFrame({id = itemId}, ItemUtils.GetItemBase(itemId)))
end
...
def.method().UpdateOwnedLQList = function(self)
  ...
  local wearLQClsId = FabaoSpiritModule.GetEquipedLQClsId()
  table.sort(self._arrOwnedLQ or {}, function(a, b)
    if a.class_id == wearLQClsId then
      return true
    elseif b.class_id == wearLQClsId then
      return false
...
def.override("userdata").onClickObj = function(self, clickObj)
  ...
  elseif string.find(id, "Group_Icon_%d") then
    local idx = tonumber(string.sub(id, #"Group_Icon_" + 1, #id))
    if self._status.selectIdx ~= idx then
      self:OnSelectLQ(idx)
    else
      clickObj:GetComponent("UIToggle").value = true
  ...
  elseif id == "Btn_ChangeModel" then
    if self.showType == ListShowType.List then
      self.showType = ListShowType.Grid
    else
      self.showType = ListShowType.List
    end
    self:SetLeftList()
...
def.method().LoadConfig = function(self)
  if LuaPlayerPrefs.HasGlobalKey(FabaoSpiritNode.CFG_LIST_TYPE) then
    self.showType = LuaPlayerPrefs.GetGlobalInt(FabaoSpiritNode.CFG_LIST_TYPE)
  else
    self.showType = ListShowType.List
  end
end
def.method().SaveConfig = function(self)
  LuaPlayerPrefs.SetGlobalInt(FabaoSpiritNode.CFG_LIST_TYPE, self.showType)
  LuaPlayerPrefs.Save()
end
