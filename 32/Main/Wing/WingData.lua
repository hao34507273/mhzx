...
def.method("table").SetWing = function(self, w)
  ...
  wing.expireTime = w.expire_time or 0
...
def.method("number", "number").SetWingExpireTime = function(self, wingId, expireTime)
  local wing = self:GetWingByWingId(wingId)
  if wing then
    wing.expireTime = expireTime
  end
end
def.method("number", "=>", "number").GetWingExpireTime = function(self, wingId)
  local wing = self:GetWingByWingId(wingId)
  if wing then
    return wing.expireTime
  end
  return 0
end
def.method("number", "=>", "boolean").IsWingForeverOwn = function(self, wingId)
  local wing = self:GetWingByWingId(wingId)
  if wing == nil then
    return false
  end
  return wing.expireTime <= 0
end
def.method("number", "=>", "boolean").IsWingExpired = function(self, wingId)
  local wing = self:GetWingByWingId(wingId)
  if wing == nil then
    return true
  elseif wing.expireTime <= 0 then
    return false
  else
    return _G.GetServerTime() > wing.expireTime
  end
end
def.method("number", "=>", "boolean").HasWing = function(self, wingId)
  if self.wings == nil then
    return false
  end
  return self.wings[wingId] ~= nil
end
