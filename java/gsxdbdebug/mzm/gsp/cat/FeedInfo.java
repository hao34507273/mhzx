/*    */ package mzm.gsp.cat;
/*    */ 
/*    */ import com.goldhuman.Common.Marshal.Marshal;
/*    */ import com.goldhuman.Common.Marshal.MarshalException;
/*    */ import com.goldhuman.Common.Marshal.OctetsStream;
/*    */ import com.goldhuman.Common.Octets;
/*    */ 
/*    */ public class FeedInfo implements Marshal
/*    */ {
/*    */   public long roleid;
/*    */   public Octets role_name;
/*    */   public int feed_timestamp;
/*    */   
/*    */   public FeedInfo()
/*    */   {
/* 16 */     this.role_name = new Octets();
/*    */   }
/*    */   
/*    */   public FeedInfo(long _roleid_, Octets _role_name_, int _feed_timestamp_) {
/* 20 */     this.roleid = _roleid_;
/* 21 */     this.role_name = _role_name_;
/* 22 */     this.feed_timestamp = _feed_timestamp_;
/*    */   }
/*    */   
/*    */   public final boolean _validator_() {
/* 26 */     return true;
/*    */   }
/*    */   
/*    */   public OctetsStream marshal(OctetsStream _os_) {
/* 30 */     _os_.marshal(this.roleid);
/* 31 */     _os_.marshal(this.role_name);
/* 32 */     _os_.marshal(this.feed_timestamp);
/* 33 */     return _os_;
/*    */   }
/*    */   
/*    */   public OctetsStream unmarshal(OctetsStream _os_) throws MarshalException {
/* 37 */     this.roleid = _os_.unmarshal_long();
/* 38 */     this.role_name = _os_.unmarshal_Octets();
/* 39 */     this.feed_timestamp = _os_.unmarshal_int();
/* 40 */     return _os_;
/*    */   }
/*    */   
/*    */   public boolean equals(Object _o1_) {
/* 44 */     if (_o1_ == this) return true;
/* 45 */     if ((_o1_ instanceof FeedInfo)) {
/* 46 */       FeedInfo _o_ = (FeedInfo)_o1_;
/* 47 */       if (this.roleid != _o_.roleid) return false;
/* 48 */       if (!this.role_name.equals(_o_.role_name)) return false;
/* 49 */       if (this.feed_timestamp != _o_.feed_timestamp) return false;
/* 50 */       return true;
/*    */     }
/* 52 */     return false;
/*    */   }
/*    */   
/*    */   public int hashCode() {
/* 56 */     int _h_ = 0;
/* 57 */     _h_ += (int)this.roleid;
/* 58 */     _h_ += this.role_name.hashCode();
/* 59 */     _h_ += this.feed_timestamp;
/* 60 */     return _h_;
/*    */   }
/*    */   
/*    */   public String toString() {
/* 64 */     StringBuilder _sb_ = new StringBuilder();
/* 65 */     _sb_.append("(");
/* 66 */     _sb_.append(this.roleid).append(",");
/* 67 */     _sb_.append("B").append(this.role_name.size()).append(",");
/* 68 */     _sb_.append(this.feed_timestamp).append(",");
/* 69 */     _sb_.append(")");
/* 70 */     return _sb_.toString();
/*    */   }
/*    */ }


/* Location:              D:\桌面\山海\gsxdbdebug.jar!\mzm\gsp\cat\FeedInfo.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       0.7.1
 */