...
/*    */ 
/*    */ public class POnPvIFightEnd extends mzm.gsp.fight.event.PVIMonsterFightEndProcedure
/*    */ {
/*    */   protected boolean processImp() throws Exception
/*    */   {
/*  9 */     for (Iterator i$ = ((mzm.gsp.fight.event.PVIMonsterFightEndArg)this.arg).escapedRoles.iterator(); i$.hasNext();) { long roleId = ((Long)i$.next()).longValue();
/* 10 */       syncRolePetProperty(roleId);
/* 11 */       xdb.Procedure.execute(new PPetLifeTooLow(roleId));
/*    */     }
/* 13 */     for (Iterator i$ = ((mzm.gsp.fight.event.PVIMonsterFightEndArg)this.arg).diedRoles.iterator(); i$.hasNext();) { long roleId = ((Long)i$.next()).longValue();
/* 14 */       syncRolePetProperty(roleId);
/* 15 */       xdb.Procedure.execute(new PPetLifeTooLow(roleId));
/*    */     }
/* 17 */     for (Iterator i$ = ((mzm.gsp.fight.event.PVIMonsterFightEndArg)this.arg).alivedRoles.iterator(); i$.hasNext();) { long roleId = ((Long)i$.next()).longValue();
...
/* Location:              D:\桌面\山海\gsxdbdebug.jar!\mzm\gsp\pet\main\POnPvIFightEnd.class
