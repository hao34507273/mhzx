...
/*    */   {
/*  9 */     for (Iterator i$ = ((mzm.gsp.fight.event.PVCFightEndArg)this.arg).activeEscapedRoles.iterator(); i$.hasNext();) { long roleId = ((Long)i$.next()).longValue();
/* 10 */       syncRolePetProperty(roleId);
/* 11 */       xdb.Procedure.execute(new PPetLifeTooLow(roleId));
/*    */     }
/* 13 */     for (Iterator i$ = ((mzm.gsp.fight.event.PVCFightEndArg)this.arg).activeDeadRoles.iterator(); i$.hasNext();) { long roleId = ((Long)i$.next()).longValue();
/* 14 */       syncRolePetProperty(roleId);
/* 15 */       xdb.Procedure.execute(new PPetLifeTooLow(roleId));
/*    */     }
/* 17 */     for (Iterator i$ = ((mzm.gsp.fight.event.PVCFightEndArg)this.arg).activeAlivedRoles.iterator(); i$.hasNext();) { long roleId = ((Long)i$.next()).longValue();
...
/* Location:              D:\桌面\山海\gsxdbdebug.jar!\mzm\gsp\pet\main\POnPvCFightEnd.class
