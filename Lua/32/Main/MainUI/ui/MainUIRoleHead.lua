...
def.method().InitUI = function(self)
  ...
  self.uiObjs.Buff_YB = self.m_node:FindDirect("Buff_YB")
  self.uiObjs.Group_Point = self.m_node:FindDirect("Group_Point")
end
...
def.method("table").SetFightProp = function(self, data)
  ...
  local maxRage = data.ragemax or heroProp:GetMaxAnger()
  local energy = data.energy or -1
  self:SetRoleHPBar(data.hp, maxHp, true)
  self:SetRoleMPBar(data.mp, maxMp, true)
  self:SetRoleAngerBar(data.rage, maxRage, true)
  self:SetRoleEnergy(energy)
end
...
def.method("number").SetRoleEnergy = function(self, energy)
  if self.uiObjs.Group_Point == nil then
    return
  end
  self.uiObjs.Group_Point:SetActive(energy >= 0)
  for i = 1, 6 do
    self.uiObjs.Group_Point:FindChild("Point0" .. i):SetActive(i <= energy)
  end
end
...
def.method().UpdateDebt = function(self)
  self.uiObjs.Buff_YB:SetActive(false)
end
...
def.static("table", "table").OnRepay = function(p1, p2)
  if instance then
    instance:UpdateDebt()
  end
end
