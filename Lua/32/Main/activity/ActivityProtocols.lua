...
def.static("table").OnSShimenDayPerfectAward = function(p)
  ...
  if awardInfo.itemMap and next(awardInfo.itemMap) then
    ...
    table.insert(personAward, {
      PersonalHelper.Type.ItemMapWithLink,
      awardInfo.itemMap
    })
  end
...
def.static("table").OnSSyncLegendTimeReward = function(p)
  ...
  local PersonalHelper = require("Main.Chat.PersonalHelper")
  PersonalHelper.CommonMsg(PersonalHelper.Type.Text, textRes.Common[150], PersonalHelper.Type.ItemMapWithLink, itemMap)
end
    ...
    for idx, cfg in pairs(weekly) do
      if cfg.id == p.activityid then
        ...
        if cfg.activityDisType == ActivityDisPlayType.Holiday then
          activityInterface._newHoliDayActivitiesSet[p.activityid] = p.activityid
          table.insert(activityInterface._newHoliDayActivitiesVector, p.activityid)
          break
        end
        if cfg.activityDisType == ActivityDisPlayType.Single then
          activityInterface._newSingleActivitiesSet[p.activityid] = p.activityid
          table.insert(activityInterface._newSingleActivitiesVector, p.activityid)
          break
        end
        if cfg.activityDisType == ActivityDisPlayType.Team then
          activityInterface._newTeamActivitiesSet[p.activityid] = p.activityid
          table.insert(activityInterface._newTeamActivitiesVector, p.activityid)
          break
        end
        if cfg.activityDisType == ActivityDisPlayType.Gang then
          activityInterface._newGangActivitiesSet[p.activityid] = p.activityid
          table.insert(activityInterface._newGangActivitiesVector, p.activityid)
        end
        break
      end
    end
...
def.static("table").OnSynActivityEnd = function(p)
  ...
  elseif cfg.activityDisType == ActivityDisPlayType.Holiday and activityInterface._newHolidayActivitiesSet[p.activityid] then
    activityInterface._newHolidayActivitiesSet[p.activityid] = nil
    local newlist = {}
    for i = 1, #activityInterface._newHolidayActivitiesVector do
      if activityInterface._newHolidayActivitiesVector[i] ~= p.activityid then
        table.insert(newlist, activityInterface._newHolidayActivitiesVector[i])
      end
    end
    activityInterface._newHolidayActivitiesVector = newlist
  elseif cfg.activityDisType == ActivityDisPlayType.Single then
    activityInterface._newSingleActivitiesSet[p.activityid] = nil
    local newlist = {}
    for i = 1, #activityInterface._newSingleActivitiesVector do
      if activityInterface._newSingleActivitiesVector[i] ~= p.activityid then
        table.insert(newlist, activityInterface._newSingleActivitiesVector[i])
      end
    end
    activityInterface._newSingleActivitiesVector = newlist
  elseif cfg.activityDisType == ActivityDisPlayType.Team then
    activityInterface._newTeamActivitiesSet[p.activityid] = nil
    local newlist = {}
    for i = 1, #activityInterface._newTeamActivitiesVector do
      if activityInterface._newTeamActivitiesVector[i] ~= p.activityid then
        table.insert(newlist, activityInterface._newTeamActivitiesVector[i])
      end
    end
    activityInterface._newTeamActivitiesVector = newlist
  elseif cfg.activityDisType == ActivityDisPlayType.Gang then
    activityInterface._newGangActivitiesSet[p.activityid] = nil
    local newlist = {}
    for i = 1, #activityInterface._newGangActivitiesVector do
      if activityInterface._newGangActivitiesVector[i] ~= p.activityid then
        table.insert(newlist, activityInterface._newGangActivitiesVector[i])
      end
    end
    activityInterface._newGangActivitiesVector = newlist
  end
...
def.static("table").OnSCurrentWeekCannotAccept = function(p)
  ...
  Event.DispatchEvent(ModuleId.ACTIVITY, gmodule.notifyId.activity.ACTIVITY_SHILIAN_OPEN_SERVER_WEEK_NO_ACCEPT, {})
end
...
def.static("table").OnSCommonErrorInfo = function(p)
  ...
  elseif p.errorCode == 1225 then
    Toast(textRes.activity.ActivityBagError[1])
  end
end
