...
local TransSexLoadingPanel = Lplus.Extend(ECPanelBase, "TransSexLoadingPanel")
local def = TransSexLoadingPanel.define
local instance
def.static("=>", TransSexLoadingPanel).Instance = function()
  if instance == nil then
    instance = TransSexLoadingPanel()
...
def.method().ShowPanel = function(self)
  ...
  local resPath = RESPATH.PREFAB_TRANS_SEX_LOADING_PANEL
...
def.method("=>", "number").GetCurrentDurationTime = function(self)
  return (self.progress - self.lastProgress) * TransSexLoadingPanel.PROGRESS_DURATION_TIME
end
...
def.method("boolean").HandleEventListeners = function(self, isRigister)
  ...
  if eventFunc then
    eventFunc(ModuleId.LOGIN, gmodule.notifyId.Login.LEAVE_WORLD_STAGE, TransSexLoadingPanel.OnLeaveWorldStage)
  end
end
...
return TransSexLoadingPanel.Commit()
