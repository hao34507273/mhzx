...
def.method("userdata").onClickObj = function(self, clickobj)
  ...
  if string.find(id, "Img_BgIcon") then
    ...
    local skillInfo = self.skills[index]
    if self.isAuto then
      if not skillInfo.canAuto then
        Toast(textRes.Fight[41])
        return
      end
      if self.unit.fightUnitType == GameUnitType.ROLE then
        fightMgr.Instance():SetAutoSkill(skillInfo.realSkillId)
      end
    else
      local usedData = self.unit.skillUsedData[skillInfo.realSkillId]
      local skillCfg = fightMgr.Instance():GetSkillCfg(skillInfo.realSkillId)
      ...
      local isValid, reason = fightMgr.Instance():CheckSkillRequirement(self.unit, skillInfo.realSkillId, skillInfo.level, skillInfo.count)
      if isValid == false and reason > 0 then
        Toast(textRes.Fight[153])
        return
      end
      fightMgr.Instance():SetAction(0, skillInfo.realSkillId)
      if not FightUtils.IsNormalAttack(self.unit.menpai, skillInfo.id) and skillInfo.id ~= constant.FightConst.DEFENCE_SKILL and skillInfo.canAuto and self.unit.fightUnitType == GameUnitType.ROLE then
        fightMgr.Instance().role_shortcut_skill = skillInfo.realSkillId
      end
      require("Main.Fight.ui.DlgFight").Instance():ShowSelectSkill(skillInfo)
    end
    self:Hide()
