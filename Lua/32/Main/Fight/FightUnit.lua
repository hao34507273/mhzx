...
local EffectDirectionEnum = require("consts.mzm.gsp.skill.confbean.EffectDirectionEnum")
local DAMAGE_TYPE = {
  HP = 1,
  MP = 2,
  RAGE = 3,
...
def.field("number").change_model_id = 0
...
def.field("number").hpInMonitor = 0
...
def.final("number", "number", "number", "table", "string", "userdata", "number", "table", "=>", FightUnit).Create = function(id, team, pos_idx, modelInfo, name, nameColor, unitType, mgr)
  ...
  inst.name = name
  modelInfo.extraMap[ModelInfo.RADIANCE_CFG_ID] = nil
  modelInfo.extraMap[ModelInfo.KITE_EFFECT_ID] = nil
...
def.method().LoadModel = function(self)
  local teamPos = self.fightMgr.fightPos[self.team].Pos
  local pos = teamPos[self.pos]
  local dir = FightConst.TeamDir[self.team]
  _G.LoadModel(self.model, self.modelInfo, pos.x, pos.y, dir, true, true)
end
def.method("number").SetOccupation = function(self, occupation)
  self.menpai = occupation
  if self.menpai == OccupationEnum.YIN_YANG_SHI and self.model then
    self.model.hasEnergy = true
    self.energy = 0
  end
end
...
def.method("table").DoPlayEffect = function(self, effres)
  ...
  for _, pos in pairs(posInfo) do
    ...
    elseif pos.playType == EffectMoveType.STATIC then
      ...
      local baseModel = self.model.m_model
      if effres.effectDirection == EffectDirectionEnum.TARGET and pos.target then
        baseModel = pos.target.model.m_model
      end
      local rotation = baseModel.localRotation
      if effres.rotate ~= 0 then
        local model_angle = baseModel.transform.eulerAngles
      ...
      if self.avatars then
        for _, avatar in pairs(self.avatars) do
          local avatar_pos = avatar:GetPos()
    ...
    elseif pos.playType == EffectMoveType.FLY then
      do
        local offsetY = 0
        if effres.attachPoint ~= BODY_PART.FEET then
        ...
        if offsetY ~= 0 and pos.toOffsetY and 0 < pos.toOffsetY then
          to = EC.Vector3.new(to.x, to.y + pos.toOffsetY, to.z)
        end
        local flyCfg = FightUtils.GetFlyEffectCfg(effres.id)
        if flyCfg then
          if flyCfg.start_offset ~= 0 then
            from = from + self.model.m_model.forward * flyCfg.start_offset
          end
          if flyCfg.end_offset ~= 0 then
            to = to - self.model.m_model.forward * flyCfg.end_offset
          end
          do
            local fx = self.fightMgr:PlayEffect(effres.path, nil, from, Quaternion.identity)
            GameUtil.AddGlobalTimer(flyCfg.beginTime, true, function()
              if _G.IsNil(fx) then
                return
              end
              FightUtils.MoveObjectToTargetPos(fx, to, flyCfg.duration, 0.1, nil)
            end)
          end
        elseif effres.behitTime and 0 < effres.behitTime then
...
def.method("table").DoSpecialAct = function(self, sp_act)
  ...
  if sp_act.act == SkillSpecialAct.AVATAR_APPEAR then
    ...
    local moveActionId = self.skill and self.skill.moveAction or 0
    ...
    for i = 2, #self.curAttackTargets do
      ...
      avatar:Play(self:GetModelAction(ActionName.FightStand))
    end
  ...
  elseif sp_act.act == SkillSpecialAct.SUMMON then
    self:SummonExtraPet(sp_act.action, sp_act.param)
  end
end
...
def.method("number", "number").SetHp = function(self, v, maxv)
  ...
  if self.hpInMonitor ~= 0 and self.hp ~= self.hpInMonitor then
    self.hpInMonitor = self.hp
    Event.DispatchEvent(ModuleId.FIGHT, gmodule.notifyId.Fight.HP_CHANGE, {
      pos = FightConst.FightPosToCfgPos[self.pos],
      hp = self.hp,
      hpMax = self.hpmax
    })
  end
end
def.method("number").SetMonitorHp = function(self, hp)
  self.hpInMonitor = hp
end
...
def.method("number").SetEnergy = function(self, v)
  if self.energy < 0 and v == 0 then
    return
  end
  self.energy = v
  self.model:SetEnergy(v)
end
...
def.method("string", "number", "number", "number", "function", "boolean", "number", "number").MoveTo = function(self, actionName, x, y, duration, callback, bSetDir, movetype, effid)
  if self.model == nil then
    self.fightMgr:AddLog(string.format("[FightLog]model is nil when %s(%d) move to dest, ", self.name, self.id))
    return
  end
  local fx
  ...
  self.model:Play(self:GetModelAction(actionName))
...
def.method().Attack = function(self)
  ...
  if target and target.counterAttack then
    ...
    return
  elseif target and target.isProtecter then
    self:AddNextAction("NextAttack", function()
      return target.isProtecter == false
    end, self.Attack)
    return
  end
  ...
  if self.skill and self.skill.boneEffectId and 0 < self.skill.boneEffectId then
    self:SetBoneEffect(self.skill.boneEffectId)
  end
  if self.skill and self.skill.cameraMoveId and 0 < self.skill.cameraMoveId then
    self.fightMgr:CameraCloseUpToTarget(self, self.skill.cameraMoveId)
  end
  if self.skill and self.skill.ghostEffId and 0 < self.skill.ghostEffId then
    self:PlayGhostEffect(self.skill.ghostEffId)
  end
  self:SetAttackDir(actModel)
  self.playTime = 0
  if self.skill and self.skill.effectDuration then
    self.playTime = self.skill.effectDuration
  end
  if self.skill and self.skill.specialActDuraction and self.playTime < self.skill.specialActDuraction then
  ...
  if not isBoomingSkill and self.skill and self.skill.actionName then
    ...
    if not actModel:HasAnimClip(actionName) then
      ...
      self.fightMgr:AddLog(string.format("action(%s) not found for model(%d)", actionName, actModel.mModelId))
    end
    local actionDuration = actModel:GetAniDuration(actionName)
    if actionDuration > self.playTime then
      self.playTime = actionDuration
    end
    local ret = actModel:PlayAnim(self:GetModelAction(actionName), function()
    ...
    if actModel.mECPartComponent then
      actModel.mECPartComponent:PlayAnimation(actionName)
    end
    if self.avatars then
      for i = 1, #self.avatars do
        self.avatars[i]:Play(self:GetModelAction(actionName))
      end
    end
    if not ret then
  ...
  if self.skill and self.skill.sound and 0 < self.skill.sound then
...
def.method().AdditionalAttack = function(self)
  ...
  if tar then
    local function DoAdditionalAttack()
      ...
      self.influences = self.additionalAttack.influenceMap[self.id]
      tar.influences = self.additionalAttack.influenceMap[tar.id]
...
def.method().EndAction = function(self)
  ...
  elseif self.counterAttack then
    ...
    elseif target and (target.isCounterAttacked or target.status.isRebound) then
      ...
      self:AddNextAction("CounterAttack", function()
  ...
  elseif target and target.hasCombo then
    ...
    if self.attack_result == nil or #self.attack_result == 0 or self.curHitInfo and self.curHitInfo.type == BEHIT_POINT_TYPE.UPDATE_STATUS then
      if (self.isDead == DEATH_STATUS.ALIVE or self.isDead == DEATH_STATUS.FAKEDEAD or self:HasDeathAction()) and self.afterStatus then
        if self.playcfg and (self.playcfg.skillType == PlaySkillEnum.DEATH_TRIGGER or self.playcfg.skillType == PlaySkillEnum.DEATH_BOOMING) and self.deathSkills then
      ...
      if self.statusList then
        if 0 < #self.statusList then
          ...
          self:CheckAbnormalStatus(false)
          self.statusList = nil
  ...
  if self.playcfg and #self.targets > 0 then
    ...
    for k, v in pairs(targets) do
      ...
      if attackResult == nil then
        ...
        if self.skill then
          skillModelId = self.skill and self.skill.modelId or -1
          skillActionId = self.skill and self.skill.actionId or -1
        end
        ...
        for unitid, infStatus in pairs(v.influences.otherMap) do
          if unitid > 0 then
            ...
            if self:CheckSpecFightStatusSet(infStatus, FighterStatus.STATUS_BLACKHOLE_DAMAGE) then
              ...
              unit:CheckAbnormalStatus(false)
            end
          end
        end
      end
      ...
      if v.protect and 0 < #v.protect.protecterids then
        local isProtected = v:CheckSpecFightStatusSet(attackResult.targetStatus, FighterStatus.STATUS_BE_PROTECT)
        if isProtected then
...
def.method(FightUnit, "table").ShowFightStatus = function(self, target, fighter_status)
  if target == nil or target.model == nil then
    return
  end
  if fighter_status == nil then
    self.fightMgr:AddLog("[FightLog]fight_status is nil for target: " .. target.name)
    return
  end
  if target.isInModelChange then
    target:AddNextAction("ShowFightStatus", function()
      return target.isInModelChange == false
    end, function()
      self:ShowFightStatus(target, fighter_status)
    end)
    return
  end
  ...
  for k, v in pairs(fighter_status.status_set) do
    ...
    elseif v == FighterStatus.STATUS_IMMUNE then
      target.model:ShowTitleIcon(9003, 1, FightModel.Title_Priority.OTHER)
    elseif v == FighterStatus.STATUS_ANTI_SEAL then
      target.model:ShowTitleIcon(9010, 1, FightModel.Title_Priority.OTHER)
    end
  end
  ...
  self:SetEnergy(self.status.curEnergy)
  ...
  local change_model_id = next(fighter_status.changeModels, nil) or 0
  if change_model_id and change_model_id ~= target.change_model_id then
    target.change_model_id = change_model_id
    target:RecoverModel(function()
      target:SetBuffStatus(fighter_status)
      target.status = fighter_status
      target:CheckAbnormalStatus(false)
      target:EndAction()
    end)
    return
  end
  ...
  target:CheckAbnormalStatus(false)
  if isMiss then
    target.model:ShowTitleIcon(9001, 1, FightModel.Title_Priority.OTHER)
  end
  if revive then
  ...
  local playDeath = true
  if target.reviveStatus then
    playDeath = self:CheckSpecFightStatusSet(target.reviveStatus, FighterStatus.STATUS_RELIVE)
  end
  if (isDead or isFakeDead) and playDeath then
  ...
  elseif isDodge then
    target.model:ShowTitleIcon(9004, 1, FightModel.Title_Priority.OTHER)
    target:Dodge()
  elseif isAttacked then
    if isAbsorb then
      target.model:ShowTitleIcon(9007, 1, FightModel.Title_Priority.OTHER)
    end
    local behitInfo = self.curHitInfo
    if behitInfo then
      behitInfo.isLastHit = isLastHit
    ...
    if hpdelta ~= 0 or fighter_status.mpchange ~= 0 then
      ...
      if not playDeath and target.reviveStatus then
        local status = target.reviveStatus
        target.reviveStatus = nil
        target:UpdateStatus(status, true)
      end
...
def.method("table", "boolean", "=>", "boolean").UpdateStatus = function(self, status, showmp)
  ...
  if self.isInModelChange then
    self:AddNextAction("ShowFightStatus", function()
      return self.isInModelChange == false
    end, function()
      self:UpdateStatus(status, showmp)
    end)
    return false
  end
  local isCritical = false
  ...
  for k, v in pairs(status.status_set) do
    ...
    elseif v == FighterStatus.STATUS_ABSORB_DAMAGE then
      isAbsorb = true
    elseif v == FighterStatus.STATUS_IMMUNE then
      self.model:ShowTitleIcon(9003, 1, FightModel.Title_Priority.OTHER)
    elseif v == FighterStatus.STATUS_HEAD or v == FighterStatus.STATUS_BODY or v == FighterStatus.STATUS_TAIL then
      state = v
    elseif v == FighterStatus.STATUS_ANGER_ADD0 then
      isSpecialRage = true
      isCritical = true
    elseif v == FighterStatus.STATUS_ANGER_REDUCE0 then
      isSpecialRage = true
    elseif v == FighterStatus.STATUS_ANTI_SEAL then
      self.model:ShowTitleIcon(9010, 1, FightModel.Title_Priority.OTHER)
    end
  end
  ...
  self:SetEnergy(status.curEnergy)
  if state > 0 then
    self.model:SetModelActionMap(self.modelId, state)
    self.model:PlayAnim(self:GetModelAction(ActionName.ChangeState), function()
      self.model.modelState = state
      self.model:Stand()
      self:EndAction()
    end)
    return true
  ...
  if isSpecialRage then
    self:ShowDamage(status.angerchange, isCritical, DAMAGE_TYPE.SPEC_RAGE)
  elseif status.angerchange ~= 0 then
    self:ShowDamage(status.angerchange, isCritical, DAMAGE_TYPE.RAGE)
  end
  local change_model_id = next(status.changeModels, nil) or 0
  if change_model_id and change_model_id ~= self.change_model_id then
    self.change_model_id = change_model_id
    self:RecoverModel(function()
      self:SetBuffStatus(status)
      self.status = status
      self:CheckAbnormalStatus(false)
      self:EndAction()
  ...
  self.status = status
  self:CheckAbnormalStatus(false)
  if self.isDead == DEATH_STATUS.FAKEDEAD and isDead then
    self.isDead = DEATH_STATUS.DEAD
    local dissolveCfg = FightUtils.GetDissolveCfg(161300001)
    if dissolveCfg then
      self:Dissolve(dissolveCfg.effId, Color.Color(dissolveCfg.R, dissolveCfg.G, dissolveCfg.B, dissolveCfg.A))
    else
      GameUtil.AddGlobalTimer(0, true, function()
        if self.fightMgr.isInFight then
          self:OnDieEnd()
...
def.method("table").SetStatus = function(self, status)
  ...
  for k, v in pairs(status.status_set) do
    ...
    elseif v == FighterStatus.STATUS_FAKE_DEAD then
      self:SetFakeDeath()
    elseif v == FighterStatus.STATUS_HEAD or v == FighterStatus.STATUS_BODY or v == FighterStatus.STATUS_TAIL then
      self.model.modelState = v
      self.model:SetModelActionMap(self.modelId, self.model.modelState)
  ...
  local function OnModelLoadDone()
    self:SetBuffStatus(status)
    self:ShowPassiveSkillTitle(status.triggerPassiveSkills)
    self.status = status
    ...
    self.model:SetStance()
    self:CheckAbnormalStatus(false)
    local isDisappeared = self:CheckBuffStatus(FighterStatus.BUFF_ST__ANTI_VISIBLE)
    if isDisappeared then
      self:SetVisible(false)
    end
  end
  local change_model_id = next(status.changeModels, nil) or 0
  if change_model_id and change_model_id ~= self.change_model_id then
    self.change_model_id = change_model_id
    self:RecoverModel(OnModelLoadDone)
  else
    OnModelLoadDone()
  end
end
  ...
  for k, v in pairs(self.damageLabels) do
    if v.delay > 0 then
      ...
      if v.time >= DAMAGE_DURATION or v.obj:get_isnil() then
        ...
        local width = v.label:GetComponent("UILabel").width
        if self.fightMgr.isPerspective then
          damage_temp_pos.x = v.initPos.x
        elseif v.initPos.x - width / 2 < -FightConst.BATTLE_FIELD_WIDTH / 2 then
          damage_temp_pos.x = (width - FightConst.BATTLE_FIELD_WIDTH) / 2
        elseif v.initPos.x + width / 2 > FightConst.BATTLE_FIELD_WIDTH / 2 then
          damage_temp_pos.x = (FightConst.BATTLE_FIELD_WIDTH - width) / 2
        else
          damage_temp_pos.x = v.initPos.x
        end
        if v.isCritical then
          damage_temp_pos.y = v.initPos.y
...
def.method("table", "boolean").ShowPropChange = function(self, status, crit)
  ...
  for _, v in pairs(status.status_set) do
    if v == FighterStatus.STATUS_ANGER_ADD0 or v == FighterStatus.STATUS_ANGER_REDUCE0 then
      isSpecialRage = true
      break
    end
  end
  ...
  if isSpecialRage then
    self:ShowDamage(status.angerchange, crit, DAMAGE_TYPE.SPEC_RAGE)
  elseif status.angerchange ~= 0 then
...
def.method("number", "boolean", "number").ShowDamage = function(self, hpDelta, isCritical, damageType)
  ...
  if critBg then
    critBg:SetActive(isCritical and damageType == DAMAGE_TYPE.HP)
  end
  ...
  if damageType == DAMAGE_TYPE.HP then
    ...
    if hpDelta > 0 then
      ...
      if self.fightMgr:tryget("CalcTotalDamage") then
        self.fightMgr:CalcTotalDamage(self, math.abs(hpDelta))
      end
    end
  ...
  elseif damageType == DAMAGE_TYPE.SPEC_RAGE then
    damageLabel.label = damageLabel.obj:FindChild("Lab_Hurt")
    local rageLabel = damageLabel.label:GetComponent("UILabel")
    rageLabel.font = fonts[FightConst.NUMBER_COLOR.YELLOW]
    if hpDelta > 0 then
    ...
    damageLabel.offset = 100
    rageLabel.text = damage
  elseif damageType == DAMAGE_TYPE.SKILL then
    damageLabel.label = damageLabel.obj:FindChild("Lab_Hurt")
    do
      local hpLabel = damageLabel.label:GetComponent("UILabel")
      hpLabel.text = ""
      damageLabel.offset = 125
      local uiTexture = critBg:GetComponent("UITexture")
      uiTexture.mainTexture = nil
      critBg:SetActive(true)
      local bundlePath = GetIconPath(damage)
      if bundlePath and bundlePath ~= "" then
        GameUtil.AsyncLoad(bundlePath, function(tex)
          if tex and uiTexture and not uiTexture:get_isnil() then
...
def.method().ShowSelect = function(self)
  ...
  if self.model.m_visible and self.model and self.model.m_selectIcon then
    ...
    local isCustomFight = self.fightMgr:IsCustomFightType()
    if isCustomFight then
      infoLabel:SetActive(true)
      info = string.format(textRes.Common[3], self.level)
      infoLabel:GetComponent("UILabel").text = info
    elseif self.fightMgr.fightType == FIGHT_TYPE.TYPE_PVP then
    ...
    if not isCustomFight and isOpen and unit then
...
def.static(FightUnit).OnAttackRunFinished = function(unit)
  ...
  local target = unit.targets and unit.targets[1]
  local protecter
  if unit.playcfg and unit.playcfg.isAttackPhase and target and target.protect and #target.protect.protecterids > 0 then
    local ret = target.attack_result[1]
    if ret then
      local isProtected = unit:CheckSpecFightStatusSet(ret.targetStatus, FighterStatus.STATUS_BE_PROTECT)
      if isProtected then
        protecter = unit.fightMgr:GetFightUnit(target.protect.protecterids[1])
        if protecter then
  ...
  if protecter and not protecter.protectReady then
    unit:AddNextAction("Protect", function()
...
def.method("table").BeHit = function(self, behitInfo)
  ...
  if self.inDefense then
    if self.skill then
      ...
      if actionCfg and 0 < actionCfg.sound then
    ...
    actModel:Play(self:GetModelAction(actionName))
  ...
  if self:CheckBuffStatus(FighterStatus.BUFF_ST__ICECOOL) or self:CheckBuffStatus(FighterStatus.BUFF_ST__STONE) then
    ...
    if self.masterModel or self.isHugeUnit then
      actModel:Play(self:GetModelAction(behitName))
    ...
    if behitInfo.behitAction == BeHitType.NULL then
      if self.hitbackInfo == nil then
        local ret = actModel:Play(self:GetModelAction(behitName))
        if not ret then
          self:BehitEnd()
          return
        end
...
def.method().HitRecovery = function(self)
  ...
  if self.hitbackInfo == nil then
    self.fightMgr:AddLog(string.format("[FightLog]%s(%d) HitRecovery, but hitbackInfo is nil", self.name, self.id))
    return
  end
...
def.method().Dodge = function(self)
  ...
  local function OnArrive()
    ...
    local function OnEnd()
      ...
      self:Stand()
      if self.dodgeInfo then
        self.model:SetDir(self.dodgeInfo.initDir)
      end
      if self.flyMount then
        self.flyMount:SwitchToBackStance(false)
      end
  ...
  if self.dodgeInfo == nil then
    self.dodgeInfo = {}
    self.dodgeInfo.initDir = self.model:GetDir()
    local angle = (270 - self.dodgeInfo.initDir) / 180 * math.pi
    ...
    local curpos = self.model:GetPos()
    local dist = math.sqrt((curpos.x - self.dodgeInfo.x) * (curpos.x - self.dodgeInfo.x) + (curpos.y - self.dodgeInfo.y) * (curpos.y - self.dodgeInfo.y))
    if dist > 50 then
...
def.method("boolean").Die = function(self, isFakeDead)
  ...
  local ret = self.model:PlayAnim(self:GetModelAction(ActionName.Death1), OnDead)
  if self.soundCfg and self.soundCfg.dead > 0 then
    self.fightMgr:PlaySoundEffect(self.soundCfg.dead)
  end
  if not ret then
...
def.method("number", "userdata").Dissolve = function(self, cfgId, color)
  local dissolveEff = GetEffectRes(cfgId)
  if dissolveEff then
    self.model:AddEffectWithRotation(dissolveEff.path, AttachType.FOOT)
    if dissolveEff.sound > 0 then
      self.fightMgr:PlaySoundEffect(dissolveEff.sound)
...
def.method().Revive = function(self)
  ...
  local function ReviveAction()
    ...
    if self:CheckFightStatus(FighterStatus.STATUS_RELIVE_TIME_BACK) then
      self.model:PlayAnim(self:GetModelAction(ActionName.Revive), OnReviveEnd)
  ...
  if effres then
    self.fightMgr:PlayEffect(effres.path, nil, self.model:Get3DPos(), Quaternion.identity)
  end
  table.insert(self.nextCallbacks, {
...
def.method("number").UpdateHitUp = function(self, dt)
  ...
  if self.fightMgr.isPerspective and 0 >= self.model.m_model.localPosition.y or not self.fightMgr.isPerspective and self.model.m_node2d.localPosition.y <= pos.y then
      ...
      if 0 < self.isDead then
        if self.model:HasAnimClip("Hit_Idle2_c") then
          ...
          local OnLand = function(self)
            local ret = self.model:PlayAnim(self:GetModelAction(ActionName.Death1), function()
              self:OnDieEnd()
            end)
            if not ret then
              self:OnDieEnd()
    ...
    local hitIdle = self:GetModelAction("Hit_Idle1_c")
    if self.model:HasAnimClip(hitIdle) then
      self.model:PlayAnim(hitIdle, OnBounceEnd)
    else
      OnBounceEnd()
    end
  end
...
def.method().GetUp = function(self)
  ...
  local revive_anim = self:GetModelAction(ActionName.Revive)
  if self.model:HasAnimClip(revive_anim) then
    self.model:PlayAnim(revive_anim, OnGetUpEnd)
  else
    self:Stand()
    self:EndAction()
  end
...
def.method("string", "number").Return = function(self, actionName, duration)
  ...
  local function OnEnd()
    if self.isProtecter then
      self.fightMgr:AddLog(string.format("[FightLog]%s(%d) protect done", self.name, self.id))
...
def.method().ResetPos = function(self)
  ...
  self:CheckAbnormalStatus(true)
end
def.method("boolean").CheckAbnormalStatus = function(self, playStand)
  ...
  elseif self:CheckBuffStatus(FighterStatus.BUFF_ST__STONE) then
    ...
    local isDisappeared = self:CheckBuffStatus(FighterStatus.BUFF_ST__ANTI_VISIBLE)
    local isInvisible = self:CheckBuffStatus(FighterStatus.BUFF_ST__INVISIBLE) and not isDisappeared
    if isInvisible then
      self.model:SetAlpha(0.5)
    elseif self.model.m_IsAlpha then
      self.model:CloseAlpha()
    end
    if playStand and self:NeedResetAction() then
      self:Stand()
    end
...
def.method().CounterAttack = function(self)
  ...
  self.model:ShowTitleIcon(9002, 1, FightModel.Title_Priority.OTHER)
...
def.method("number", "=>", "table").SetAdditionalPhase = function(self, skillId)
  ...
  if playcfg.nameEffectId > 0 then
    self.model:ShowTitleIcon(playcfg.nameEffectId, 1, FightModel.Title_Priority.SKILL)
  ...
  if phase then
    ...
    if 0 < phase[2] then
          ...
          if effPlay then
            ...
            if atkEffId > 0 then
              ...
              if effres then
                ...
                effres.effectDirection = effPlay.effectDirection
      ...
      if playcfg then
        playcfg.isAttackPhase = 0 < #self.beHitPoints
      end
    end
  end
  return playcfg
...
def.method().Join = function(self)
  ...
  self.model:Play(ActionName.Run)
  fxEscape = self.model:AddChildEffect(RESPATH.ESCAPE_EFFECT, AttachType.FOOT, "", 0)
  self.model:MoveTo(pos.x, pos.y, OnRunEnd, joinDuration, true, 0)
end
def.method(FightUnit).DoProtect = function(self, target)
  ...
  self.fightMgr:AddLog(string.format("[FightLog]%s(%d) protect %s(%d)", self.name, self.id, target.name, target.id))
  ...
  self.model:ShowTitleIcon(7009, 1, FightModel.Title_Priority.OTHER)
  target.model:ShowTitleIcon(7009, 1, FightModel.Title_Priority.OTHER)
end
...
def.method(FightUnit, "boolean").Capture = function(self, target, success)
  ...
  local function OnCaptureRunFinished()
    ...
    self:GetActModel():PlayAnim(self:GetModelAction(ActionName.Magic), OnCaptureEnd)
    if self.model.mECWingComponent then
      self.model.mECWingComponent:Attack()
    end
    if self.model.mECPartComponent then
      self.model.mECPartComponent:PlayAnimation(ActionName.Magic)
    end
...
def.method("table").Summon = function(self, action)
  ...
  local function OnSummonEnd()
    if action.result == PlaySummon.SUMMON then
      ...
      for k, v in pairs(action.fighters) do
        ...
        if self.fightMgr:tryget("GetMyPet") and unit == self.fightMgr:GetMyPet() then
          Event.DispatchEvent(ModuleId.FIGHT, gmodule.notifyId.Fight.MODEL_PROP_CHANGED, {
          ...
          elseif unit.fightUnitType == GameUnitType.CHILDREN and self.fightMgr:tryget("summonChildTimes") then
            ...
            if 1 < self.fightMgr.summonChildTimes then
              self.fightMgr.summonPetTimes = self.fightMgr.summonPetTimes - constant.FightConst.SECONDLY_SUMMON_CHILD_COST_SUMMON_PET_TIMES
            end
          end
        end
      end
  ...
  self:GetActModel():PlayAnim(self:GetModelAction(ActionName.Magic), OnSummonEnd)
  if self.model.mECWingComponent then
    self.model.mECWingComponent:Attack()
  end
  if self.model.mECPartComponent then
  ...
  for k, v in pairs(action.fighters) do
    ...
    if effcfg.sound > 0 then
      self.fightMgr:PlaySoundEffect(effcfg.sound)
    end
  end
end
def.method("table", "number").SummonExtraPet = function(self, action, summonDelay)
  local PlayExtraSummon = require("netio.protocol.mzm.gsp.fight.PlayExtraSummon")
  local teamId = FightConst.ACTIVE_TEAM
  if action.team == PlayExtraSummon.PASSIVE_TEAM then
    teamId = FightConst.PASSIVE_TEAM
  end
  local teamPos = self.fightMgr.fightPos[teamId].Pos
  local function OnSummonEnd()
    if action.result == PlayExtraSummon.SUMMON then
      local pet = self.fightMgr.groups[self.group].extra_pet
      if pet then
        self.fightMgr:RemoveFightUnit(pet.id)
        self.fightMgr.groups[self.group].extra_pet = nil
      end
      self.fightMgr:WaitForNewCreatedUnits()
      for k, v in pairs(action.fighters) do
        self.fightMgr:CreateFighter(k, v, teamId, action.groupid)
      end
    elseif action.result == PlayExtraSummon.SUMMON_BACK then
      for k, _ in pairs(action.fighters) do
        self.fightMgr:RemoveFightUnit(k)
      end
    end
  ...
  local effcfg = GetEffectRes(702010101)
  for k, v in pairs(action.fighters) do
    local effpos = teamPos[v.pos + 1]
    local u3dpos = Map2DPosTo3D(effpos.x, effpos.y)
    self.fightMgr:PlayEffect(effcfg.path, nil, u3dpos, Quaternion.identity)
  ...
  if summonDelay > 0 then
    GameUtil.AddGlobalTimer(summonDelay / 1000, true, OnSummonEnd)
  else
    OnSummonEnd()
  end
end
def.method("table").Talk = function(self, action)
  ...
  if self.fightUnitType == GameUnitType.MONSTER and self.fightMgr:tryget("AddUnitTalkToChannel") then
    self.fightMgr:AddUnitTalkToChannel(self, content)
  end
end
def.method("table", "table", "table", "table").UseItem = function(self, status, targets, targetStatus, influences)
  ...
  local function OnEffEnd()
    self.fightMgr:AddLog(string.format("[FightLog]%s(%d) update item status", self.name, self.id))
    ...
    if influences then
      for uid, fight_status in pairs(influences) do
        local unit = self.fightMgr:GetFightUnit(uid)
        if unit then
          unit:UpdateStatus(fight_status, true)
        end
      end
    end
  ...
  local function OnActEnd()
    self:GetActModel():CrossFade(self:GetModelAction(ActionName.FightStand), 0.1)
    local EffectPosType = require("consts.mzm.gsp.skill.confbean.EffectPosType")
    for _, v in pairs(targets) do
      local effres = GetEffectRes(702020005)
      if effres then
  ...
  self:GetActModel():PlayAnim(self:GetModelAction(ActionName.Magic), OnActEnd)
  if self.model.mECWingComponent then
    self.model.mECWingComponent:Attack()
  end
  if self.model.mECPartComponent then
...
def.method("number", "number").PlaySkill = function(self, skillId, skillType)
  ...
  if self.targets and self.playcfg.skillPlayType == SkillAttackType.SINGLE then
    local phasesNum = #self.playcfg.phases
    local targetsNum = 0
    for i = 1, #self.targets do
      local target = self.targets[i]
      local atk_results = self.fightMgr:GetFightUnitAttackResult(target.id)
      targetsNum = targetsNum + #atk_results
      local attack_others = atk_results[1].attackOthers
    ...
    for i = 1, phasesNum do
      ...
      local behitNum = 0
      if phaseid > 0 then
        behitNum = self:GetBehitPoint(phaseid)
      end
      if behitNum > 0 then
    ...
    if target then
      target.attack_result = self.fightMgr:GetFightUnitAttackResult(target.id)
    end
  elseif self.targets and self.playcfg.skillPlayType == SkillAttackType.MULTI then
    for i = 1, #self.targets do
      local target = self.targets[i]
      target.attack_result = self.fightMgr:GetFightUnitAttackResult(target.id)
    end
  ...
  if 0 < playcfg.prepare then
    ...
    if phaseCfg then
      ...
      if action then
        ...
        self:GetActModel():PlayAnim(self:GetModelAction(action.name), OnEnd)
        if self.model.mECWingComponent then
          self.model.mECWingComponent:Attack()
        end
      end
...
def.method("table", "table").Play = function(self, action, targets)
  ...
  if action.actionType == PlayType.PLAY_SKILL then
    ...
    self.influences = action.influenceMap[self.id]
    if action.deathfighter2Skills then
      for uid, ds in pairs(action.deathfighter2Skills) do
        local unit = self.fightMgr:GetFightUnit(uid)
        if unit then
          unit.deathSkills = ds.skillList
        end
      end
    end
  ...
  elseif action.actionType == PlayType.PLAY_EXTRA_SUMMON then
    for k, v in pairs(action.fighter_status) do
      local unit = self.fightMgr:GetFightUnit(k)
      if unit then
        unit:UpdateStatus(v, true)
      end
    end
    self.specialAct = {}
    self.specialAct.action = action
    self:PlaySkill(constant.FightConst.SUMMON_EXTRA_PET_SKILL_ID, PlaySkillEnum.NORMAL)
  end
end
...
-- fail 97
unluac.decompile.expression.TableReference@14c265e
-1
-- fail 523
unluac.decompile.expression.TableReference@8139f0
-1
    ...
    elseif phase[1] > 0 then
      do
        ...
        if moveAction.modelDisPlay == MovePlayType.DISAPPEAR_SLOWLY then
          local actionName = self:GetModelAction(moveAction.actionName)
          if self.model:HasAnimClip(actionName) then
            self.model:Play(actionName)
          else
            self.model:Play(self:GetModelAction(ActionName.Magic))
          end
          self.valishTime = {
...
def.method("number", "number", "number", "number", "=>", "table").GetEffectPos = function(self, effectMoveType, effectPosType, attachPoint, effDest)
  ...
  local pos = {}
  if effectPosType == EffectPosType.MAIN_TARGET then
    local p = {}
    p.from = targets[1].model:GetPos()
    p.playType = EffectMoveType.STATIC
    p.target = targets[1]
    pos[1] = p
  elseif effectPosType == EffectPosType.TARGET then
    for i = 1, #targets do
      local p = {}
  ...
  if effectMoveType == EffectMoveType.NONE and (effDest == effectPosType or #pos == 0) or effectMoveType ~= EffectMoveType.NONE and effectMoveType ~= EffectMoveType.FLY and effectMoveType ~= EffectMoveType.LINK then
    return pos
  end
  if effDest == EffectPosType.MAIN_TARGET then
    pos[1].to = targets[1].model:GetPos()
    local offsetY = targets[1].model:GetBodyPartHeight(AttachType.BODY)
    if offsetY > 0 then
      pos[1].toOffsetY = offsetY
    end
    pos[1].playType = EffectMoveType.FLY
  elseif effDest == EffectPosType.TARGET then
    for i = 1, #targets do
      local p = pos[i]
      if p == nil then
        p = {}
...
def.method("table").SetBuffStatus = function(self, status)
  ...
  for k, v in pairs(status.buffs) do
    if v.buffid > 0 then
      ...
      if buffEffectCfg then
        ...
        if 0 < buffEffectCfg.modelScale then
          local scale = buffEffectCfg.modelScale
          if model_scale < scale then
            model_scale = scale
          end
        end
      end
  ...
  if model_scale > 0 then
    self.model:ChangeScale(model_scale)
  end
  for k, v in pairs(self.childEffects) do
    if not v.exist then
      ECFxMan.Instance():Stop(v.fx)
  ...
  for k, tmpBuff in pairs(buff_data) do
    if 0 > tmpBuff.buffid then
      ...
      elseif tmpBuff.buffid == Buff.MIRROR_FIGHTER_ID then
        if self.change_model_id == 0 then
          self:ChangeToUnit(tmpBuff.round, self.state)
        else
          self.appearanceId = tmpBuff.round
        end
      ...
      elseif tmpBuff.buffid <= Buff.FIGHT_STATE_START and tmpBuff.buffid >= Buff.FIGHT_STATE_END then
        if self.state == FightStateGroupState.YINENG_ULTIMATE and tmpBuff.round ~= FightStateGroupState.YINENG_ULTIMATE then
          if self.change_model_id == 0 then
            self:ChangeToUnit(self.id, tmpBuff.round)
          end
        elseif self.state ~= FightStateGroupState.YINENG_ULTIMATE and tmpBuff.round == FightStateGroupState.YINENG_ULTIMATE and self.change_model_id == 0 then
      ...
      elseif tmpBuff.buffid == Buff.SEAL_TIPS then
        self:ShowDamage(9008, false, DAMAGE_TYPE.SKILL)
      end
    end
  end
...
def.method("boolean").SetReady = function(self, isReady)
  if isReady then
    self:SetOnlineStatusIcon()
    self.in_turn = false
  else
    self.model:SetDefaultTitleIcon(7011)
  end
end
...
def.method().ClearTargets = function(self)
  self.targets = nil
  self.curAttackTargets = nil
end
...
def.method("number").RemoveTitleIcon = function(self, iconId)
  if iconId > 0 then
    self.model:RemoveTitleIcon(iconId)
  end
end
def.method().SetOnlineStatusIcon = function(self)
  if self.online then
    self.model:SetDefaultTitleIcon(0)
  else
    self.model:SetDefaultTitleIcon(7010)
  end
end
...
def.method("=>", "boolean").CheckSkillValid = function(self)
  ...
  elseif self.playcfg.skillPlayType == SkillAttackType.SINGLE then
    for i = 1, #self.targets do
      local attack_results = self.fightMgr:GetFightUnitAttackResult(self.targets[i].id)
      if attack_results then
        local attackResult = attack_results[1]
        if attackResult then
          fightStatusNum = fightStatusNum + #attack_results
          local attack_others = attackResult.attackOthers
          if attack_others and #attack_others > 0 then
            fightStatusNum = fightStatusNum + #attack_others
          end
        end
      end
    end
...
def.method("table").Evolve = function(self, action)
  ...
  self:GetActModel():PlayAnim(self:GetModelAction(ActionName.Magic), OnEvolveEnd)
  if self.model.mECWingComponent then
    self.model.mECWingComponent:Attack()
  end
  if self.model.mECPartComponent then
...
def.method("table").ChangeModel = function(self, action)
  ...
  self:GetActModel():PlayAnim(self:GetModelAction(ActionName.Magic), BeginChangeModel)
  if self.model.mECWingComponent then
    self.model.mECWingComponent:Attack()
  end
  if self.model.mECPartComponent then
...
def.method("number", "table", "function").DoModelChange = function(self, modelId, modelInfo, cb)
  ...
  local initModelInfo = self.model.initModelInfo
  self.model:Destroy()
  self.fightMgr:WaitForNewCreatedUnits()
  ...
  self.model.initModelInfo = initModelInfo
  self.modelId = modelId
  self.model:SetHpVisible(hpvisible)
  self:SetOccupation(self.menpai)
...
def.method("number").ShowSkillTitle = function(self, skillId)
  if skillId > 0 then
    ...
    if record then
      ...
      if title > 0 then
        self.model:ShowTitleIcon(title, 1, FightModel.Title_Priority.SKILL)
      end
    end
  end
end
...
def.method("function").RecoverModel = function(self, cb)
  local function on_change_model_end()
    if cb then
      _G.SafeCall(cb)
    end
  end
  if self.change_model_id > 0 then
    if self.change_model_id == self.modelId then
      on_change_model_end()
      return
    end
    local modelInfo = {
      modelid = self.change_model_id
    }
    self:DoModelChange(self.change_model_id, modelInfo, on_change_model_end)
  elseif 0 < self.appearanceId and self.appearanceId ~= self.id then
    local unit = self.fightMgr:GetFightUnit(self.appearanceId)
    if unit then
      self:DoModelChange(unit.modelInfo.modelid, unit.modelInfo, on_change_model_end)
    end
  elseif self.state == FightStateGroupState.YINENG_ULTIMATE then
    local changeModelCfg = FightUtils.GetSkillChangeModelCfg(self.state, self.gender)
    if changeModelCfg then
      local modelInfo = {
        modelid = changeModelCfg.modelId
      }
      self:DoModelChange(modelInfo.modelid, modelInfo, on_change_model_end)
    end
  else
    self:DoModelChange(self.modelInfo.modelid, self.modelInfo, on_change_model_end)
  end
end
def.method().RemoveAllCallbacks = function(self)
  self.nextCallbacks = {}
end
def.method("boolean").SetVisible = function(self, v)
  ...
  if self.flyMount then
    self.flyMount:SetVisible(v)
  end
end
def.method("number").ReturnOnException = function(self, skillId)
  local returnType = FightUtils.GetSkillReturnTypeOnException(skillId)
  local moveAction = FightUtils.GetMoveActionCfg(returnType)
  if moveAction == nil then
    self.fightMgr:AddLog(string.format("[Fight error]ReturnOnException got nil move action for skillId : %d", skillId))
    table.insert(self.nextCallbacks, {
      delay = 0.1,
      callback = self.EndAction
    })
  ...
  if moveAction.modelDisPlay == MovePlayType.APPEAR_SLOWLY then
    self.appearTime = {
      time = moveAction.duration,
      duration = moveAction.duration
    }
...
def.method("string", "=>", "string").GetModelAction = function(self, actionName)
  if self.model == nil then
    return actionName
  end
  return self.model:GetModelAction(actionName)
end
FightUnit.Commit()
return FightUnit
