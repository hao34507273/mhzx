...
def.override().OnCreate = function(self)
  ...
  Event.RegisterEventWithContext(ModuleId.FEATURE, gmodule.notifyId.Feature.FeatureOpenChange, InventoryDlg.OnFunctionOpenChange, self)
...
def.override().OnDestroy = function(self)
  ...
  Event.UnregisterEvent(ModuleId.FEATURE, gmodule.notifyId.Feature.FeatureOpenChange, InventoryDlg.OnFunctionOpenChange)
...
def.method("userdata").onClickObj = function(self, obj)
  ...
  if id == "Btn_Item" and string.find(obj.parent.name, "item_") then
    ...
    local strs = string.split(Label_Name.name, "_")
    local ocp = tonumber(strs[3])
    local gender = tonumber(strs[4])
    self:SelectEquipmentOcp(ocp, gender)
  else
    self:onClick(id)
  end
end
        ...
        if lv > myLv then
          ...
          if not bShowFabao then
            ...
            if equipLQClsId and equipLQClsId > 0 then
              ...
              ItemTipsMgr.Instance():ShowFabaoLQWearTips(ownedLQInfo, itemBase, ItemTipsMgr.Source.Equip, 0, 0, 0, 0, 0, true, true)
            end
          end
        end
      else
        local ocp = self:GetSelectedOccupation()
        local gender = self:GetSelectedGender()
        local key, item = self._viewModel:GetEquipmentByPosition(ocp, gender, index)
        if key == -1 then
          ...
          tips:SetOperateContext({ocp = ocp, gender = gender})
        end
      end
    end
  ...
  if string.find(id, "Item_") then
    ...
    if index then
      ...
      elseif self._state == InventoryDlg.ShowState.Bag then
        ...
        local ocp = self:GetSelectedOccupation()
        local gender = self:GetSelectedGender()
        local context = {ocp = ocp, gender = gender}
        if ope then
...
def.method().UpdateModel = function(self)
  ...
  local ocp = self:GetSelectedOccupation()
  local gender = self:GetSelectedGender()
  local modelId = self._viewModel:GetOccupationModelId(ocp, gender)
  if self.model ~= nil then
    self.model:Destroy()
  end
  self.model = ECUIModel.new(modelId)
  local modelInfo = self._viewModel:GetOccupationModelInfo(ocp, gender)
...
def.method("table").OnFunctionOpenChange = function(self, params)
  ...
  if params.feature == ModuleFunSwitchInfo.TYPE_GENDER_CONVERT or params.feature == ModuleFunSwitchInfo.TYPE_MULTI_OCCUPATION then
    self:UpdateOcpEquipGroup()
  end
end
...
def.method().UpdateOcpEquipGroup = function(self)
  ...
  local ownedOccpGenders = self._viewModel:GetOwnOccpGenderInfos()
  local isOpen = self._viewModel:IsMultiOccupationOpen() and ownedOccpGenders and #ownedOccpGenders > 1
  if not isOpen then
  ...
  local selGender = self:GetSelectedGender()
  local occupationName = require("Main.TransSex.TransSexUtils").GetOccpGenderName(selOcp, selGender)
  GUIUtils.SetText(Label_Current, occupationName)
  self:CloseOcpChooseList()
  self:UpdateTransSexEqpsEffect()
end
def.method().OpenOcpChooseList = function(self)
  require("Main.TransSex.TransSexModule").Instance():ConsumeEqpsReddot()
  self:UpdateTransSexEqpsEffect()
  ...
  local ownedOccupationInfos = self._viewModel:GetOwnOccpGenderInfos()
  ...
  for i, childGO in ipairs(childGOs) do
    ...
    Label_Name.name = "Label_Name_" .. occupationInfo.occupation .. "_" .. occupationInfo.gender
    local occupationName = occupationInfo:GetName()
...
def.method("number", "number").SelectEquipmentOcp = function(self, ocp, gender)
  local selOcp = self:GetSelectedOccupation()
  local selGender = self:GetSelectedGender()
  if selOcp == ocp and selGender == gender then
    self:CloseOcpChooseList()
    return
  end
  self._viewModel:AsyncLoadOccupationEquipments(ocp, gender, function(equipments)
    if self.m_panel == nil then
      return
    end
    self.selOcp = ocp
    self.selGender = gender
    if self._state == InventoryDlg.ShowState.Storage then
      return
    end
    if ocp ~= _G.GetHeroProp().occupation or gender ~= _G.GetHeroProp().gender then
...
def.method("=>", "number").GetSelectedGender = function(self)
  if self.selGender == 0 then
    self.selGender = _G.GetHeroProp().gender
  end
  return self.selGender
end
def.method("=>", "table").GetSelectedOcpEquipments = function(self)
  local occupation = self:GetSelectedOccupation()
  local selGender = self:GetSelectedGender()
  return self._viewModel:GetOccupationEquipments(occupation, selGender) or {}
end
def.method().UpdateTransSexEqpsEffect = function(self)
  local img = self.m_panel:FindDirect("Img_Bg0/Img_BgBag/Group_Zhiye/Img_Bg")
  local bReddot = require("Main.TransSex.TransSexModule").Instance():NeedEqpsReddot()
  if bReddot then
    GUIUtils.SetLightEffect(img, GUIUtils.Light.Square)
  else
    GUIUtils.SetLightEffect(img, GUIUtils.Light.None)
  end
end
