...
def.method("number").TryCandidate = function(self, service)
  ...
  if teamData.Instance():HasTeam() then
    local isTmpLeave = teamData.Instance():GetStatus() == require("netio.protocol.mzm.gsp.team.TeamMember").ST_TMP_LEAVE
    if not isTmpLeave then
...
def.method("userdata", "string")._canvassInGang = function(self, roleId, text)
  ...
  local textOctets, textPackOctets = ChatUtils.UnpackChatContentToOctets(text)
  gmodule.network.sendProtocol(require("netio.protocol.mzm.gsp.menpaistar.CGangCanvass").new(roleId, textOctets, textPackOctets))
end
...
def.method("userdata", "string", "boolean")._canvassInWorld = function(self, roleId, text, useYb)
  ...
  local ChatUtils = require("Main.Chat.ChatUtils")
  local textOctets, textPackOctets = ChatUtils.UnpackChatContentToOctets(text)
  gmodule.network.sendProtocol(require("netio.protocol.mzm.gsp.menpaistar.CWorldCanvass").new(roleId, useYb and 1 or 0, myYuanbao, textOctets, textPackOctets))
end
def.method("userdata", "string", "string", "number", "=>", "string").GetCanvassText = function(self, roleId, name, text, money)
  local ChatInfoPackHelper = require("Main.Chat.ChatInfoPackHelper")
  local cname = ChatInfoPackHelper.GetColorInfoPack(name, "ff8000")
  local link = ChatInfoPackHelper.GetMenpaiStarVoteInfoPack2(roleId)
  if money > 0 then
    local award = ChatInfoPackHelper.GetColorInfoPack(tostring(money), "ff8000")
