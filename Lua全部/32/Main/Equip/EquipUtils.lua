...
def.static("number", "number", "=>", "number").GetQiLingMaxLevelWithPos = function(equipLevel, wearpos)
  local record = DynamicData.GetRecord(CFG_PATH.DATA_EQUIP_TRANS_INHERIT_CFG, equipLevel)
  if record == nil then
    warn("GetQiLingMaxLevel  failed~~~~~")
    return -1
  ...
  local qilingMax = record:GetIntValue("qilingMaxLevel")
  local posMaxLvl = -1
  local rec2 = record:GetStructValue("wearpos2maxstrlvlStruct")
  local sz = rec2:GetVectorSize("wearpos2maxstrlvlVector")
  for i = 1, sz do
    local rec3 = rec2:GetVectorValueByIdx("wearpos2maxstrlvlVector", i - 1)
    local pos = rec3:GetIntValue("wearpos")
    local lvl = rec3:GetIntValue("maxLevel")
    if pos == wearpos then
      posMaxLvl = lvl
      break
    end
  ...
  if posMaxLvl == -1 then
    warn("Get QiLing pos " .. wearpos .. " MaxLevel  failed~~~~~")
  end
  return math.min(qilingMax, posMaxLvl)
end
...
def.static("number", "=>", "table").GetMakeItemTable = function(makeId)
  ...
  equip.desItem = {}
  local recDest = makeCfgRecord:GetStructValue("desItemStruct")
  size = recDest:GetVectorSize("desItemVector")
  for i = 0, size - 1 do
    local rec = recDest:GetVectorValueByIdx("desItemVector", i)
    local itemId = rec:GetIntValue("itemId")
    table.insert(equip.desItem, itemId)
  end
  return equip
end
...
def.static("number", "=>", "table").GetEquipBasicInfo = function(eqpId)
  ...
  equip.assistModel = equipRecord:GetIntValue("assistModel") or 0
...
def.static("table", "=>", "number").CalcEquipShenShiScore = function(item)
  local score = 0
  if item and item.shenshi_plan_info and item.shenshi_plan_info[1] then
    local planKey = item.shenshi_plan_info[1].current_plan
    local shenShiInfo = item.shenshi_plan_info[1].plan_2_current[planKey]
    if shenShiInfo and shenShiInfo.un_used == require("netio.protocol.mzm.gsp.item.ShenShiInfo").USED then
      score = score + shenShiInfo.prop_value * EquipUtils.GetPropertyFactor(shenShiInfo.prop_type)
    end
  end
  return score
end
...
def.static("=>", "boolean").hasEquipCanStren = function()
  ...
  for key, bagInfo in pairs(equipBagInfo) do
    ...
    if itemBase ~= nil and itemBase.itemType == ItemType.EQUIP then
      ...
      local wearpos = equipBase.wearpos
      if strenMinLv <= itemUseLevel then
        local curStrenLevel = EquipUtils.GetEquipStrenLevel(eqpBagId, key)
        local maxStrenLevel = EquipUtils.GetQiLingMaxLevelWithPos(itemUseLevel, wearpos)
...
def.static("table", "=>", "boolean").canStren = function(equipInfo)
  ...
  local wearpos = equipInfo.wearPos or equipInfo.wearpos
  ...
  local maxStrenLevel = EquipUtils.GetQiLingMaxLevelWithPos(useLevel, wearpos)
  if curStrenLevel >= maxStrenLevel then
    return false
  end
...
def.static("number", "=>", "table", "string").GetAllQilingCfgByLevel = function(level)
  ...
  for i = 0, size - 1 do
    ...
    local propParam = rec:GetFloatValue("param")
    if propType > 0 then
  ...
  if allQlCfg then
    ...
    for k, v in ipairs(allQlCfg) do
      ...
      elseif propValue < 1 then
        str = string.format(textRes.Equip[135], propName, propValue, lowestEquipLevel)
      else
...
def.static("number", "number").RequestEquipmentSchemaInfo = function(bagId, key)
  local p = require("netio.protocol.mzm.gsp.equipmentschema.CGetEquipmentSchemaInfoReq").new(bagId, key)
  gmodule.network.sendProtocol(p)
end
def.static("number", "number", "=>", "string").GetPlanDesc = function(bagId, key)
  local EquipSchemeMgr = require("Main.Equip.EquipSchemeMgr")
  local planId, desc = EquipSchemeMgr.Instance():getSelectedIdAndCustomname(bagId, key)
  warn(bagId, key)
  warn("planId, desc", planId, desc)
  if planId <= 0 then
    return textRes.Equip[617]
  end
  local MathHelper = require("Common.MathHelper")
  return string.format(textRes.Equip[616], MathHelper.Arabic2Chinese(planId), desc)
end
EquipUtils.Commit()
return EquipUtils
