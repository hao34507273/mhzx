...
local dlg
def.field("table").TopAlignFrame = nil
def.field("boolean").IsAdjusted = false
...
def.static("table", "string", "number", "boolean", "number", "number", "string", "string", OperationBase, "table", "table", "string", "table", "number", "=>", ItemTips).ShowTipInfo = function(pos, title, iconId, isEquiped, state, level, equipType, desc, topOperation, bottomOperation, itemBase, panelName, extInfo, godWeaponStage)
  local tip = ItemTips()
  tip.position = pos
  tip.title = title
  tip.iconId = iconId
...
def.method().UpdateInfo = function(self)
  ...
  GameUtil.AddGlobalLateTimer(0.01, true, function()
    ...
    if self.position.auto then
      ...
      if self.TopAlignFrame and self.TopAlignFrame.IsAdjusted then
        local relY = self.TopAlignFrame.m_panel:FindDirect("Table_Tips").transform.localPosition.y
        if adjustY < relY then
          adjustY = relY
        else
          local relFrame = self.TopAlignFrame.m_panel:FindDirect("Table_Tips")
          local newPos = Vector.Vector3.new(relFrame.transform.localPosition.x, adjustY, 0)
          relFrame:set_localPosition(newPos)
        end
      end
      tipFrame:set_localPosition(Vector.Vector3.new(self.position.x, adjustY, 0))
    end
    self.IsAdjusted = true
    self.TopAlignFrame = nil
...
def.method("number").SetNewIcon = function(self, iconId)
  self.iconId = iconId
...
def.method("=>", "table").GetItemBase = function(self)
...
def.method().UpdateTitle = function(self)
  local uiTexture = self.m_panel:FindDirect("Table_Tips/Title/Img_Item/Img_Icon"):GetComponent("UITexture")
  GUIUtils.FillIcon(uiTexture, self.iconId)
  local itemBase = self:GetItemBase()
...
def.method("string").onClick = function(self, id)
  ...
  elseif string.find(id, "lingbao_") then
    local skillId = tonumber(string.sub(id, #"lingbao_" + 1))
    if skillId == 0 then
      Toast(textRes.Pet.Gen2[10])
    elseif skillId == 1 then
      Toast(textRes.Pet.Gen2[11])
    else
      local source = self.m_panel:FindDirect("Table_Tips")
      local position = source:get_position()
      local screenPos = WorldPosToScreen(position.x, position.y)
  ...
  elseif string.find(id, "item_") then
    ...
    local sprite = self.m_panel:FindDirect("Table_Tips"):GetComponent("UISprite")
    local ItemTipsMgr = require("Main.Item.ItemTipsMgr")
    ItemTipsMgr.Instance():ShowBasicTipsRename(itemId, screenPos.x, screenPos.y - sprite:get_height() / 2, sprite:get_width(), sprite:get_height(), 0, true, self.panelName .. "_embed")
  elseif string.find(id, "commonskill_") then
    local skillId = tonumber(string.sub(id, #"commonskill_" + 1))
    local source = self.m_panel:FindDirect("Table_Tips")
    local position = source:get_position()
    local screenPos = WorldPosToScreen(position.x, position.y)
    local sprite = self.m_panel:FindDirect("Table_Tips"):GetComponent("UISprite")
  ...
  elseif string.find(id, "appellation_") then
    local id = tonumber(string.sub(id, #"appellation_" + 1))
    require("Main.title.TitleMgr").ShowChengweiTipsById(id)
  elseif string.find(id, "title_") then
    local id = tonumber(string.sub(id, #"title_" + 1))
    require("Main.title.TitleMgr").ShowTouxianTipsById(id)
