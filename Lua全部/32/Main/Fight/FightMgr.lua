...
local mainCam, tipResume, tipLoading
...
def.field("userdata").fight_bg_eff = nil
...
def.field("table").checkAttrGroupList = nil
def.field("table").waitForTargets = nil
def.field("table").nextAction = nil
def.field("table").curAction = nil
...
def.field("number").totalDamage = 0
...
def.method().Init = function(self)
  ...
  gmodule.network.registerProtocol("netio.protocol.mzm.gsp.fight.SEnterFightExtraInfoBrd", FightMgr.OnSEnterFightExtraInfoBrd)
  gmodule.network.registerProtocol("netio.protocol.mzm.gsp.fight.SFightEndExtraInfoBrd", FightMgr.OnSFightEndExtraInfoBrd)
...
def.static("table").OnSEnterFight = function(p)
  ...
  local worldBossCfg, worldBossActionCfg
  if p.fight.fight_type == FIGHT_TYPE.TYPE_PVIMonster and 0 < p.fight.fight_cfg_id then
    worldBossCfg = FightUtils.GetWorldBossCfg(p.fight.fight_cfg_id)
    worldBossActionCfg = FightUtils.GetWorldBossActionCfg(p.fight.fight_cfg_id)
  end
  ...
  instance:CreateFightTeam(p.fight.active_team, FightConst.ACTIVE_TEAM, true)
  instance:CreateFightTeam(p.fight.passive_team, FightConst.PASSIVE_TEAM, true)
  ...
  if p.fight.fight_type == FIGHT_TYPE.TYPE_PVIMonster then
    if worldBossCfg then
      instance:Load3DSceneLight(worldBossCfg.lightMap)
    end
  ...
  if not isInRelaying then
    Event.DispatchEvent(ModuleId.FIGHT, gmodule.notifyId.Fight.ENTER_FIGHT, {
      ...
      fightCfgId = instance.fightCfgId,
      fightType = fight_category
    })
  ...
  instance.rounds = {}
  local passive_round = bit.rshift(p.fight.round, 24)
  local active_round = bit.band(bit.rshift(p.fight.round, 16), 255)
  local big_round = passive_round + active_round - 1
  if big_round < 0 then
    big_round = 0
  end
  instance.curRound = bit.band(p.fight.round, 65535)
  instance.curServerRound = instance.curRound
  if 0 < instance.curRound then
    Event.DispatchEvent(ModuleId.FIGHT, gmodule.notifyId.Fight.FIGHT_START, nil)
  ...
  if worldBossActionCfg then
    instance:SetWorldBossInfo(worldBossActionCfg)
  end
  ...
  if fight_category == FightCategory.WORLD_BOSS_OCTOPUS then
    for _, unit in pairs(instance.fightUnits) do
      if unit.team ~= instance.myTeam then
        unit.isHugeUnit = true
      end
    end
  end
  if instance.sceneInfo.useFightChannel then
    local ChatMsgData = require("Main.Chat.ChatMsgData")
    require("Main.Chat.ui.ChannelChatPanel").Instance():SetDefaultChannel(ChatMsgData.MsgType.CHANNEL, ChatMsgData.Channel.FIGHT)
  end
  if fight_category == FightCategory.BOSS or fight_category == FightCategory.WORLD_BOSS_OCTOPUS then
    local damage = 0
    for _, unit in pairs(instance.fightUnits) do
      if unit.team ~= instance.myTeam then
        damage = unit.hpmax - unit.hp
        if damage > 0 then
          instance.totalDamage = instance.totalDamage + damage
        end
      end
    end
  end
  ...
  DlgFight.Instance():SetBigRound(big_round)
  ...
  if cmdTime > 0 then
    if not instance.auto_fight_status or cmdTime > constant.FightConst.WAIT_CMD_TIME - constant.FightConst.AUTO_WAIT_TIME then
      local roundStartTime = p.fight.operEndTime - constant.FightConst.WAIT_CMD_TIME * 1000
      table.insert(instance.rounds, {
        big_round = big_round,
        round_num = instance.curRound,
        cmd_time = roundStartTime,
        readyList = readyList
      })
      instance:DoRoundCommand()
    end
  elseif 0 < instance.curRound then
    ...
    GameUtil.AddGlobalTimer(2, true, function()
      if instance.isInFight and instance.curServerRound == 0 then
        tipLoading = require("GUI.CommonUITipsDlg").ShowConstTip(textRes.Fight[93], {x = 0, y = 0})
        tipLoading.textAlign = Alignment.Center
      end
    end)
  end
...
def.method("number", "boolean").LoadFightBg = function(self, bgId, showMask)
  ...
  if respath == nil or respath == "" then
    Debug.LogWarning(string.format("fight bg path is nil or empty for id: %d", bgId))
    return
  end
  GameUtil.AsyncLoad(respath, function(obj)
    ...
    if self.fight_bg == nil then
      ...
      local cam = ECGame.Instance().m_fightCam:GetComponent("Camera")
      local orthographicSize = cam.orthographicSize
      if cam.aspect <= 2 then
        self.fight_bg.localScale = EC.Vector3.one * orthographicSize * 4
      else
        self.fight_bg.localScale = EC.Vector3.one * orthographicSize * 2 * cam.aspect
      end
...
def.method("number", "boolean").PlayFightBgEffect = function(self, effId, goNext)
  local function OnPlayEnd()
    if not self.isInFight then
      return
    end
    self:SetShakeParam(161100000, 0)
    self.shakeParam.amplitude = 0.025
    self.shakeParam.time = 1
    self.shakeParam.duration = 1
    if goNext then
      GameUtil.AddGlobalTimer(1, true, function()
        if not self.isInFight then
          return
  ...
  if self.fight_bg_eff and self.fight_bg_eff.name ~= tostring(effId) then
    self.fight_bg_eff:Destroy()
    self.fight_bg_eff = nil
  end
  if self.fight_bg_eff == nil then
    local effPath = _G.GetEffectRes(effId)
    local respath = effPath and effPath.path
    if respath == nil or respath == "" then
      Debug.LogWarning(string.format("fight bg eff path is nil or empty for id: %d", effId))
      return
    end
    GameUtil.AsyncLoad(respath, function(obj)
      if not self.isInFight then
      ...
      if not _G.IsNil(obj) then
        if self.fight_bg_eff then
          self.fight_bg_eff:Destroy()
        end
        self.fight_bg_eff = Object.Instantiate(obj, "GameObject")
        self.fight_bg_eff:SetLayer(ClientDef_Layer.Fight)
        self.fight_bg_eff.name = tostring(effId)
        self.fight_bg_eff.transform.parent = self.fightSceneNode.transform
        self.fight_bg_eff.localPosition = EC.Vector3.zero
        local orthographicSize = ECGame.Instance().m_fightCam:GetComponent("Camera").orthographicSize
        self.fight_bg_eff.localScale = EC.Vector3.one * orthographicSize * 4
        if goNext then
          GameUtil.AddGlobalTimer(2, true, OnPlayEnd)
        end
      elseif goNext then
        self:GoNext()
      end
    ...
    self.fight_bg_eff:SetActive(false)
    self.fight_bg_eff:SetActive(true)
    if goNext then
      GameUtil.AddGlobalTimer(2, true, OnPlayEnd)
    end
  end
end
def.method("number").Load3DSceneLight = function(self, id)
  local respath = _G.GetModelPath(id)
  if respath == nil then
    return
  end
  GameUtil.AsyncLoad(respath, function(obj)
...
local LoadingRespathToTeam
local formationRotationVector = {
  EC.Vector3.new(0, 180, 0),
  EC.Vector3.new(0, 180, 180)
}
def.method("string", "number", "boolean").LoadFormationIcon = function(self, respath, team, showFormationEff)
  if LoadingRespathToTeam == nil then
    LoadingRespathToTeam = {}
  end
  LoadingRespathToTeam[respath] = team
  GameUtil.AsyncLoad(respath, function(obj)
    if LoadingRespathToTeam ~= nil then
      LoadingRespathToTeam[respath] = nil
    end
    if not self.isInFight then
      return
    end
    ...
    if LoadingRespathToTeam ~= nil and next(LoadingRespathToTeam) == nil then
      LoadingRespathToTeam = nil
      if showFormationEff then
        instance:PlayFormationRelationEffect()
      end
    end
  end)
end
def.method().PlayFormationRelationEffect = function(self)
  local activeTeam = instance.teams[FightConst.ACTIVE_TEAM]
  local passiveTeam = instance.teams[FightConst.PASSIVE_TEAM]
  if instance.teams == nil then
    return
  end
  if activeTeam == nil then
    return
  end
  if passiveTeam == nil then
    return
  end
  local activeRelation = gmodule.moduleMgr:GetModule(ModuleId.FORMATION):GetFormationRelation(activeTeam.formation, passiveTeam.formation)
  if activeRelation ~= FightConst.FORMATION_RELATION.NO_RELATION then
    do
      local effres
      if activeRelation == FightConst.FORMATION_RELATION.KE_ZHI then
        effres = GetEffectRes(FightConst.FORMATION_EFFECT.KE_ZHI)
      elseif activeRelation == FightConst.FORMATION_RELATION.BEI_KE then
        effres = GetEffectRes(FightConst.FORMATION_EFFECT.BEI_KE)
      end
      local teamCenter = instance.fightPos[FightConst.ACTIVE_TEAM].Attack_center
      local u3dpos = Map2DPosTo3D(teamCenter.x, teamCenter.y)
      local function callback()
        self:PlayEffect(effres.path, nil, u3dpos, Quaternion.identity)
      end
      local isRightBottom = FightConst.RIGHT_BOTTOM == FightConst.ACTIVE_TEAM
      DlgFight.Instance():ShowFormationRelationEffect(isRightBottom, activeRelation, callback)
    end
  end
  local passiveRelation = gmodule.moduleMgr:GetModule(ModuleId.FORMATION):GetFormationRelation(passiveTeam.formation, activeTeam.formation)
  if passiveRelation ~= FightConst.FORMATION_RELATION.NO_RELATION then
    do
      local effres
      if passiveRelation == FightConst.FORMATION_RELATION.KE_ZHI then
        effres = GetEffectRes(FightConst.FORMATION_EFFECT.KE_ZHI)
      elseif passiveRelation == FightConst.FORMATION_RELATION.BEI_KE then
        effres = GetEffectRes(FightConst.FORMATION_EFFECT.BEI_KE)
      end
      local teamCenter = instance.fightPos[FightConst.PASSIVE_TEAM].Attack_center
      local u3dpos = Map2DPosTo3D(teamCenter.x, teamCenter.y)
      local function callback()
        self:PlayEffect(effres.path, nil, u3dpos, Quaternion.identity)
      end
      local isRightBottom = FightConst.RIGHT_BOTTOM == FightConst.PASSIVE_TEAM
      DlgFight.Instance():ShowFormationRelationEffect(isRightBottom, passiveRelation, callback)
    end
  end
end
local spri_asset
...
def.static("table").OnSRoundStart = function(p)
  local passive_round = bit.rshift(p.round, 24)
  local active_round = bit.band(bit.rshift(p.round, 16), 255)
  local big_round = passive_round + active_round - 1
  if big_round < 0 then
    big_round = 0
  end
  instance.curServerRound = bit.band(p.round, 65535)
  ...
  if round_data then
    round_data.cmd_time = p.round_start_timestamp
  else
    table.insert(instance.rounds, {
      big_round = big_round,
      round_num = instance.curServerRound,
      cmd_time = p.round_start_timestamp
    })
  end
...
def.method().DoRoundCommand = function(self)
  ...
  self.itemList = nil
  if not self.isSpectator and self:IsShowCmdLog() then
    local tip = string.format(textRes.Fight[100], self.curRound)
    gmodule.moduleMgr:GetModule(ModuleId.CHAT):SendSystemMsg(ChatMsgData.System.FIGHT, HtmlHelper.Style.Fight, {content = tip})
  end
...
def.static("table").OnSEnterFightExtraInfoBrd = function(p)
  if instance.checkAttrGroupList == nil then
    instance.checkAttrGroupList = {}
  end
  local group_list_data = p.extra_infos[p.ALLOW_TEAMMATE_QUERY_PEOPERTY_GROUP_LIST]
  if group_list_data then
    local group_list = UnmarshalBean(require("netio.protocol.mzm.gsp.fight.IntList"), group_list_data)
    if group_list then
      instance.checkAttrGroupList[p.fightid:tostring()] = group_list.list
    end
  end
end
def.static("table").OnSFightEndExtraInfoBrd = function(p)
end
...
def.static("table").OnSSynRolePetSkillInfo = function(p)
  ...
  p.skillMap[constant.FightConst.ATTACK_SKILL] = 1
  p.skillMap[constant.FightConst.DEFENCE_SKILL] = 1
  if skilldata.petSkills == nil then
    skilldata.petSkills = {}
  end
  skilldata.petSkills[p.petuuid:tostring()] = p.skillMap
end
...
def.method("table").PlayTeamJoin = function(self, action)
  self:AddLog("[FightLog]PlayTeamJoin")
  local bigRound = action.data[action.DATA_WHEEL_COUNT]
  if bigRound and bigRound > 0 then
    DlgFight.Instance():ShowBigRoundEffect(bigRound)
  end
  self:RemoveJoinTimer()
  local curMembers = {}
  local newFighters = {}
  for camp, teamData in pairs(action.team) do
    local teamId = FightConst.ACTIVE_TEAM
    if camp == action.PASSIVE_TEAM then
      teamId = FightConst.PASSIVE_TEAM
    end
    local members = self.teams[teamId]:GetAllMembers()
    for mid, member in pairs(members) do
      if not member:IsReallyDead() then
        curMembers[mid] = member
      end
    end
    self:CreateFightTeam(teamData, teamId, false)
    DlgFight.Instance():ShowFormation()
    for mid, m in pairs(members) do
      if curMembers[mid] == nil and not m:IsReallyDead() then
        m:PrepareToJoin()
        newFighters[mid] = m
  ...
  self.join_timer_id = GameUtil.AddGlobalTimer(0.1, false, function()
    ...
    load_time = load_time - 0.1
    local isAllLoaded = load_time <= 0
    if not isAllLoaded then
      isAllLoaded = true
      for _, fighter in pairs(newFighters) do
        if fighter.model:IsInLoading() then
          isAllLoaded = false
          break
        end
      end
    ...
    if isAllLoaded then
      if load_time <= 0 then
        self:AddLog("[FightLog](PlayTeamJoin) load model time out")
        for _, m in pairs(newFighters) do
          if m:IsInLoading() then
            self:AddLog(string.format("[FightLog](PlayTeamJoin) model is not loaded: %s(%d)", m.name, m.id))
          end
        end
      end
      for _, m in pairs(newFighters) do
      ...
      local duration = constant.FightConst.ESCAPE_ACTION_TIME / 400
      GameUtil.AddGlobalTimer(duration, true, function()
        if not self.isInFight then
          return
        end
        self:PlayFormationRelationEffect()
      end)
      self:RemoveJoinTimer()
    end
  end)
...
def.method().PlayNext = function(self)
  ...
  if action.actionType == PlayType.PLAY_CHANGE_FIGHT_MAP then
    self:AddLog("[FightLog]PLAY_CHANGE_FIGHT_MAP")
    self:LoadFightBg(action.mapSource, false)
    local effId = action.data[action.DATA_CHANGE_EFFECT]
    if effId and effId > 0 then
      self.playTime = 10
      self:PlayFightBgEffect(effId, true)
    else
      self:GoNext()
    end
    return
  ...
  if action.actionType == PlayType.PLAY_TEAM_JOIN then
    self.playTime = 20
    self:PlayTeamJoin(action)
    return
  end
  if action.actionType == PlayType.PLAY_KICK_OUT then
    self:RemoveFightUnit(action.fighterid)
    self:GoNext()
    return
  end
  if action.actionType == PlayType.PLAY_ADD_BUFF then
    self:AddLog("[FightLog]PLAY_ADD_BUFF")
  ...
  if action.actionType == PlayType.PLAY_FIGHTER_STATUS then
    if next(action.fightermap) == nil then
      self:GoNext()
      return
    end
    self:AddLog("[FightLog]PLAY_FIGHTER_STATUS")
    for k, v in pairs(action.fightermap) do
      ...
      if unit then
        ...
        for _, status in pairs(v.statuses) do
          for _, stset in pairs(status.status_set) do
            ...
            elseif stset == FighterStatus.STATUS_RELIVE then
              self.playTime = self.playTime + actModel:GetAniDuration(ActionName.Revive) + FightConst.PLAY_TIME.REVIVE_DELAY
            elseif stset == FighterStatus.STATUS_HEAD or stset == FighterStatus.STATUS_BODY or stset == FighterStatus.STATUS_TAIL then
              self.playTime = self.playTime + actModel:GetAniDuration(ActionName.ChangeState) + FightConst.PLAY_TIME.CALLBACK_DELAY * 2
            end
          end
        end
        unit:ShowStatusChange(v.statuses)
      else
        Debug.LogWarning(string.format("[Fight]unit not found when PLAY_FIGHTER_STATUS: %d", k))
  ...
  local unit = self:GetFightUnit(action.fighterid)
  if unit == nil then
    Debug.LogWarning(string.format("[Fight](playnext)action unit not found: %d", action.fighterid))
    return
  end
  unit:Reset()
  local actModel = unit.masterModel or unit.model
  table.insert(self.waitForTargets, unit.id)
  if action.actionType == PlayType.PLAY_TARGET_NOTSUIT then
    unit:ReturnOnException(action.skillCfgId)
    return
  end
  local targets = {}
  if action.actionType == PlayType.PLAY_USEITEM then
    local target_str = ""
    local reviveDuration = 0
    local influences
    for k, v in pairs(action.targetStatus) do
      if k > 0 then
        ...
        for _, st in pairs(v.status_set) do
          if st == FighterStatus.STATUS_RELIVE then
            local reviveTime = FightConst.PLAY_TIME.REVIVE_DELAY + targetActModel:GetAniDuration(ActionName.Revive)
            if reviveDuration < reviveTime then
              reviveDuration = reviveTime
            end
          end
        end
        table.insert(targets, target)
        target_str = string.format(target_str .. " %s(%d)", target.name, target.id)
      elseif k < 0 then
        if influences == nil then
          influences = {}
        end
        influences[math.abs(k)] = v
      end
    end
    self:AddLog(string.format("[FightLog]%s(%d) use item to %s", unit.name, unit.id, target_str))
    self.playTime = self.playTime + reviveDuration + actModel:GetAniDuration(ActionName.Magic) + FightConst.PLAY_TIME.USE_ITEM
    unit:UseItem(action.releaserStatus, targets, action.targetStatus, influences)
    return
  end
  self:AddLog(string.format("[FightLog]%s(%d) play action %d ", unit.name, unit.id, action.actionType))
  ...
  for i = 1, #action.targets do
    ...
    if target then
      ...
      table.insert(targets, target)
      local targetAttackResult = action.status_map[target.id].attackResultBeans
      if targetAttackResult then
        for _, attackResult in pairs(targetAttackResult) do
      ...
      Debug.LogWarning(string.format("[fatal error]target could not be found for id: %d", action.targets[i]))
  ...
  if self.nextAction and self.nextAction.actionType == PlayType.PLAY_SKILL then
    ...
    for k, v in pairs(action.targets) do
      if v == unit.id or unitsInNextAction == nil or unitsInNextAction[v] ~= nil then
        table.insert(self.waitForTargets, v)
      end
    end
  else
    for k, v in pairs(action.targets) do
...
def.method("number", "=>", "table").GetFightUnitAttackResult = function(self, unitId)
  if self.curAction == nil or self.curAction.status_map == nil then
    return
  end
  local targetAttackResult = self.curAction.status_map[unitId]
  return targetAttackResult and targetAttackResult.attackResultBeans
end
def.method("table", "table", "table", "=>", "number", "number").CalcAttackResultTime = function(self, attackResult, actModel, targetActModel)
  ...
  if reviveStatus then
    reviveDuration = FightConst.PLAY_TIME.REVIVE_DELAY + targetActModel:GetAniDuration(ActionName.Revive)
  end
...
def.method("table").SetConsecutiveAttack = function(self, unit)
  ...
  if target then
    ...
    target.protect = unit.next_target_data.protect
    target.influences = unit.next_target_data.influenceOthers
    if target.protect then
      for _, protecter in pairs(target.protect.protecterids) do
        table.insert(self.waitForTargets, protecter)
        self.playTime = self.playTime + FightConst.PLAY_TIME.PROTECT_RUN * 2 + FightConst.PLAY_TIME.HITBACK_DEFAULT_DURATION
...
def.method("number", "userdata", "=>", "table").UnmarshalAction = function(self, actionType, octets)
  ...
  elseif actionType == PlayType.PLAY_ADD_BUFF then
    action = require("netio.protocol.mzm.gsp.fight.PlayAddBuff").new()
  elseif actionType == PlayType.PLAY_EXTRA_SUMMON then
    action = require("netio.protocol.mzm.gsp.fight.PlayExtraSummon").new()
  elseif actionType == PlayType.PLAY_KICK_OUT then
    action = require("netio.protocol.mzm.gsp.fight.PlayKickout").new()
  elseif actionType == PlayType.PLAY_TARGET_NOTSUIT then
    action = require("netio.protocol.mzm.gsp.fight.PlayTargetNotSuit").new()
  end
  if action == nil then
    return nil
  end
...
def.static("table", "=>", "boolean").OnFightEnd = function(p)
  ...
  if instance.sceneInfo.useFightChannel then
    local ChatMsgData = require("Main.Chat.ChatMsgData")
    require("Main.Chat.ui.ChannelChatPanel").Instance():SetDefaultChannel(ChatMsgData.MsgType.CHANNEL, ChatMsgData.Channel.CURRENT)
  end
  local fight_category = instance.sceneInfo and instance.sceneInfo.battleType or -1
  local FightCategory = require("consts.mzm.gsp.fight.confbean.FightType")
...
def.method().Reset = function(self)
  ...
  if instance.fightInstanceId then
    ...
    if instance.checkAttrGroupList then
      instance.checkAttrGroupList[str] = nil
    end
...
def.method("table", "number", "boolean").CreateFightTeam = function(self, teamData, teamid, showFormationEff)
  ...
  if formationCfg then
    ...
    if resPath then
      self:LoadFormationIcon(resPath, team.teamId, showFormationEff)
    else
  ...
  for i, g in pairs(teamData.groups) do
    ...
    if group.isControllable then
      local totalSummonPetTimes = bit.rshift(g.summonPettimes, 16)
      if totalSummonPetTimes == 0 then
        totalSummonPetTimes = constant.FightConst.SUMMON_TIMES
      end
      self.summonPetTimes = totalSummonPetTimes - bit.band(g.summonPettimes, 65535)
      self.summonChildTimes = g.summonChldtimes
      local totalUseItemTimes = bit.rshift(g.useitemtimes, 16)
      local usedItemTimes = bit.band(g.useitemtimes, 65535)
      if totalUseItemTimes == 0 then
        totalUseItemTimes = constant.FightConst.DRAG_USE_TIMES
      end
      self.useItemTimes = totalUseItemTimes - usedItemTimes
...
def.method("number", "table", "number", "number", "=>", FightUnit).CreateFighter = function(self, id, fighter, teamId, groupId)
  ...
  local isExtraPet = bit.rshift(fighter.fighter_type, 16) > 0
  if isExtraPet then
    fighter.fighter_type = bit.band(fighter.fighter_type, 255)
  end
  ...
  elseif fighter.fighter_type == GameUnitType.PET or fighter.fighter_type == GameUnitType.CHILDREN then
    ...
    if isExtraPet then
      group.extra_pet = unit
    else
      group.pet = unit
    end
    if group.isControllable and not isExtraPet then
  ...
  elseif fighter.fighter_type == GameUnitType.FELLOW then
    unit.roleId = fighter.uuid
  elseif fighter.fighter_type == GameUnitType.MONSTER or fighter.fighter_type == GameUnitType.CUSTOMIZED_FIGHTER then
    unit.cfgId = fighter.cfgid
  end
  group[unit.id] = unit
  unit.group = groupId
...
def.method("number", "table", "number", "=>", FightUnit).CreateFightUnit = function(self, fighterId, fighterInfo, team)
  ...
  unit:SetOccupation(fighterInfo.occupation)
  ...
  unit:SetEnergy(fighterInfo.status.curEnergy)
...
def.method().NextCommand = function(self)
  ...
  if unit then
    if self.rounds and #self.rounds > 0 then
      local start_time = self.rounds[1].cmd_time
      local cmdTime = 0
      if start_time then
        local curTime = GetServerTime()
        local remainTime = start_time / 1000 - curTime
        cmdTime = remainTime:ToNumber()
        if cmdTime > 0 then
          cmdTime = 0
        end
      end
      DlgFight.Instance():StartCountDown(constant.FightConst.WAIT_CMD_TIME + cmdTime)
    end
...
def.method("table").OnActionEnd = function(self, params)
  local unit_id = params[1]
  local unit = params.unit or instance:GetFightUnit(unit_id)
  local islast = params[2]
  ...
  if unit then
    ...
    if group then
      if group.role == unit then
        group.role = nil
      elseif group.pet == unit then
        group.pet = nil
      elseif group.extra_pet == unit then
        group.extra_pet = nil
      end
      group[unit.id] = nil
    end
  ...
  if unit.model and unit.model:IsLoaded() and unit.flyMount and not unit.flyMount:IsLoaded() and not unit.flyMount:IsInLoading() then
    ...
    if unit.fightUnitType == GameUnitType.ROLE then
      ...
      if flymountId > 0 then
        ...
        if feijianCfg then
          isChangeModel = FightUtils.IsChangeModelFlyMount(feijianCfg)
...
def.method().WaitForNewCreatedUnits = function(self)
  if self.fightUnits then
    for k, v in pairs(self.fightUnits) do
      if v and v.model and v.model:IsLoaded() then
  ...
  if self.isInUpdate and self.tobeAdded then
    for k, v in pairs(self.tobeAdded) do
      if v and v.model and v.model:IsLoaded() then
        if self.hasLoaded == nil then
          self.hasLoaded = {}
...
def.method("number", "userdata").Summon = function(self, act, id)
  ...
  local cmd
  if act == ACT_TYPE.OP_SUMMON_PET or act == ACT_TYPE.OP_SUMMON_EXTRA_PET then
    cmd = OpSummonPet.new(id)
  elseif act == ACT_TYPE.OP_SUMMON_CHILD then
    cmd = OpSummonChild.new(id)
  else
    self:AddLog(string.format("Undefined summon type : %d", act))
    return
  end
...
def.static("table").OnSSelectOperateBrd = function(p)
  ...
  if unit then
    ...
    local msg = instance:GetCommandTip(unit, p.op_type, p.content, p.auto == 1)
    if msg ~= "" then
      Toast(msg)
      if instance:IsShowCmdLog() then
        gmodule.moduleMgr:GetModule(ModuleId.CHAT):SendSystemMsg(ChatMsgData.System.FIGHT, HtmlHelper.Style.Fight, {content = msg})
      end
    end
  end
end
def.method(FightUnit, "number", "userdata", "boolean", "=>", "string").GetCommandTip = function(self, unit, cmd_type, content, isAuto)
  ...
  local function GetUnitName(unit)
    ...
    if unit.fightUnitType == GameUnitType.PET then
      ...
      unit_name = string.format(textRes.Fight[72], master.name)
    elseif unit.fightUnitType == GameUnitType.CHILDREN then
      local group = self.groups[unit.group]
      local master = group.role
      unit_name = string.format(textRes.Fight[73], master.name)
    end
    return unit_name
  end
  local function GetTargetName(targetId)
    local main_target = self:GetFightUnit(targetId)
    local targetname = GetUnitName(main_target)
  ...
  elseif cmd_type == ACT_TYPE.OP_SUMMON_PET then
    msg = string.format(textRes.Fight[75], unitname)
  elseif cmd_type == ACT_TYPE.OP_SUMMON_EXTRA_PET then
    msg = string.format(textRes.Fight[85], unitname)
  elseif cmd_type == ACT_TYPE.OP_SUMMON_CHILD then
    msg = string.format(textRes.Fight[76], unitname)
...
def.method("number", "number").SetShakeParam = function(self, id, delay)
  ...
  if self.shakeParam then
    self.shakeParam.duration = self.shakeParam.duration * 2
    self.shakeParam.time = self.shakeParam.duration
...
def.method("number").UpdateShake = function(self, tick)
  ...
  if 0 < self.shakeParam.time then
    self.shakeParam.time = self.shakeParam.time - tick
    local game = ECGame.Instance()
    local h = game.m_2DWorldCam.orthographicSize
    local w = h * 2 * game.m_2DWorldCam.aspect
    local cam = mainCam:GetComponent("Camera")
    local a = self.shakeParam.amplitude * (1 - math.random(2)) * (self.shakeParam.time / self.shakeParam.duration)
    shake_rect.x = a
    shake_rect.y = a
    cam.rect = shake_rect
    local random_size = (math.random(32) - 16) * (self.shakeParam.time / self.shakeParam.duration)
...
def.method().StopPerspective = function(self)
  ...
  mainCam.localPosition = pos3d - mainCam.forward * 15
  mainCam.localScale = EC.Vector3.one
  local cam = mainCam:GetComponent("Camera")
...
def.static("table", "table").OnLeaveWorld = function(p1, p2)
  ...
  if instance.isInFight then
    local FightCategory = require("consts.mzm.gsp.fight.confbean.FightType")
    if instance.sceneInfo and instance.sceneInfo.useFightChannel then
      local ChatMsgData = require("Main.Chat.ChatMsgData")
      require("Main.Chat.ui.ChannelChatPanel").Instance():SetDefaultChannel(ChatMsgData.MsgType.CHANNEL, ChatMsgData.Channel.CURRENT)
    end
    instance:Reset()
...
def.method("=>", FightUnit).GetMyExtraPet = function(self)
  local me = self.controllableUnits and self.controllableUnits[1]
  if me == nil then
    return nil
  end
  local group = self.groups[me.group]
  if group == nil then
    return nil
  end
  return group.extra_pet
end
...
def.method().SetAdaptedPos = function(self)
  ...
  FightConst.BATTLE_FIELD_WIDTH = w
...
def.method("number", "number", "=>", FightUnit).GetFightUnitByPos2 = function(self, team, pos)
  local unit
  if self.fightUnits then
    for k, v in pairs(self.fightUnits) do
      if v.team == team and v.pos == pos then
        unit = v
        break
      end
    end
...
def.method(FightUnit, "number", "number", "number", "=>", "boolean", "number").CheckSkillRequirement = function(self, unit, skillId, skillLevel, count)
  ...
  if costInfo.reqList then
    for k, v in pairs(costInfo.reqList) do
      ...
      elseif k == ConditionType.MP_PERCENT_SMALL_THAN then
        ret = ret and unit.mp / unit.mpmax <= v / 10000
      elseif k == ConditionType.ENERGY_BIG_THAN then
        ret = ret and v <= unit.energy
        if not ret then
          reason = ConditionType.ENERGY_BIG_THAN
        end
      elseif k == ConditionType.ENERGY_SMALL_THAN then
        ret = ret and v >= unit.energy
      end
      if not ret then
        return false, reason
  ...
  if costInfo.costList then
    for k, v in pairs(costInfo.costList) do
      ...
      elseif k == CostType.HPFIX then
        ret = ret and v <= unit.hp
      elseif k == CostType.ENERGY_FIX then
        ret = ret and v <= unit.energy
        if not ret then
          reason = CostType.ENERGY_FIX
        end
      end
      if not ret then
        return false, reason
...
def.method("=>", "table").GetPetSkillList = function(self)
  ...
  return skillList and skillList.petSkills and skillList.petSkills[tostring(pet.roleId)]
end
...
def.method(FightUnit, "table", "=>", "number").CalcPlayTime = function(self, unit, action)
  ...
  elseif action.actionType == PlayType.PLAY_EXTRA_SUMMON then
    time = time + self:CalcSkillTime(unit, constant.FightConst.SUMMON_EXTRA_PET_SKILL_ID)
    ...
    if phase then
      if 0 < phase[2] then
        ...
        if actionPhaseCfg.specialAct and 0 < actionPhaseCfg.specialAct then
          time = time + actionPhaseCfg.specialActDuration
        end
      end
      if 0 < phase[1] then
...
def.method("string", "table", "dynamic", "function", "number", "number", "number", "=>", "userdata").PlayMovingEffect = function(self, resName, pos, dest, cb, speed, duration, tolerance)
  pos = pos + FightMgr.BattleFieldPos
  dest = dest + FightMgr.BattleFieldPos
  return self:DoPlayMovingEffect(resName, pos, dest, cb, speed, duration, tolerance)
end
def.method("string", "table", "dynamic", "function", "number", "number", "number", "=>", "userdata").DoPlayMovingEffect = function(self, resName, pos, dest, cb, speed, duration, tolerance)
...
def.method(FightUnit, "number").CalcTotalDamage = function(self, unit, damage)
  if unit.team ~= self.myTeam then
    self.totalDamage = self.totalDamage + damage
    Event.DispatchEvent(ModuleId.FIGHT, gmodule.notifyId.Fight.TOTAL_DAMAGE_CHANGED, nil)
  end
end
def.method(FightUnit, "string").AddUnitTalkToChannel = function(self, unit, content)
  if self.isSpectator then
    return
  end
  ...
  local msg
  if unit.team == self.myTeam then
    msg = string.format(textRes.Fight[83], unit.name, content)
  else
    msg = string.format(textRes.Fight[84], unit.name, content)
  end
  ChatModule.Instance():SendNoteMsg(msg, ChatMsgData.MsgType.CHANNEL, ChatMsgData.Channel.CURRENT)
end
def.method("=>", "boolean").IsCustomFightType = function(self)
  local fight_category = self.sceneInfo and self.sceneInfo.battleType or 0
  return FightUtils.IsCustomFightType(fight_category)
end
def.method("number", "=>", "boolean").IsCheckAttrAvailable = function(self, unitId)
  if instance.checkAttrGroupList == nil then
    return false
  end
  local group_list = instance.checkAttrGroupList[tostring(instance.fightInstanceId)]
  if group_list == nil then
    return false
  end
  local fightUnit = self:GetFightUnit(unitId)
  local groupId = fightUnit and fightUnit.group
...
def.method("=>", "boolean").IsShowCmdLog = function(self)
  return self.sceneInfo ~= nil and self.sceneInfo.displayOperateInfoInSystemChannel == true
end
def.method("number", "=>", "number", "number").MonitorUnitHpByPos = function(self, pos)
  local fightPos = FightConst.CfgPosToFightPos[pos] or 0
  local unit = self:GetFightUnitByPos2(FightConst.PASSIVE_TEAM, fightPos)
  if unit == nil then
    return 0, 0
  end
  unit:SetMonitorHp(unit.hp)
  return unit.hp, unit.hpmax
end
FightMgr.Commit()
return FightMgr
