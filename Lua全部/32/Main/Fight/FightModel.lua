...
def.field("userdata").m_energyPate = nil
...
def.field("number").statusTitleIcon = 0
def.field("number").curTitleIcon = 0
def.field("number").titlePriority = FightModel.Title_Priority.NONE
def.field("number").flagIcon = 0
def.field("number").hp = 1
def.field("number").mp = 1
def.field("number").energy = 0
def.field("boolean").hasEnergy = false
...
def.field("table").model_offset = nil
def.field("number").model_scale = 1
def.field("table").scaleInfo = nil
def.const("number").CHANGE_SCALE_DURATION = 1.5
def.field("number").modelState = 0
def.field("table").modelActionMap = nil
...
def.override().Destroy = function(self)
  ...
  if self.m_energyPate then
    self.m_energyPate:Destroy()
    self.m_energyPate = nil
  end
...
def.override().OnLoadGameObject = function(self)
  ...
  if self.model_offset then
    model.localPosition = model.localPosition + self.model_offset
  end
  ...
  if self.hasEnergy then
    pate:CreateEnergyBoard(self)
  end
  if not self.showModel then
    self:SetShowModel(self.showModel)
  end
  if self.mECPartComponent then
    self.mECPartComponent.defaultAction = ActionName.FightStand
  end
...
def.override().SetStance = function(self)
  ...
  elseif not self.isDead then
    if self.m_IsStone then
      self:PlayAnimAtTime(self:GetModelAction(self.stand_stance), 0.1)
      self:StopCurrentAnim()
    else
      self:Play(self:GetModelAction(self.stand_stance))
    end
  else
    self:PlayAnimAtTime(self:GetModelAction(ActionName.Death1), 1)
    if self.mECWingComponent then
...
def.method().Stand = function(self)
  if self.m_IsStone then
    ...
    self:Play(self:GetModelAction(self.stand_stance))
    if self.mECPartComponent then
      self.mECPartComponent:PlayAnimation(ActionName.FightStand)
    end
  end
  if self.mECWingComponent then
...
def.method("number").SetHp = function(self, hp)
  ...
  self.m_uiHpHandle:SetActive(self.m_visible)
...
def.method("number").SetEnergy = function(self, energy)
  self.energy = energy
  if self.m_energyPate == nil or self.m_energyPate:get_isnil() then
    return
  end
  self.m_energyPate:SetActive(self.m_visible)
  local Group_Point = self.m_energyPate:FindChild("Group_Point")
  for i = 1, 6 do
    Group_Point:FindChild("Point0" .. i):SetActive(i <= self.energy)
  end
end
...
def.method("number", "number", "number").ShowTitleIcon = function(self, iconId, duration, priority)
  if _G.IsNil(self.m_topIcon) then
    self.topIcons = {}
    self.topIcons.id = iconId
    self.topIcons.duration = duration
    self.topIcons.priority = priority
    return
  end
  if priority <= self.titlePriority then
    if duration <= 0 then
      duration = FightModel.Label_Time_Type.PERMANENT
    end
    self.labelTime = duration
    self:SetTitleIconWithPriority(iconId, priority)
  elseif self.topIcons == nil then
    self.topIcons = {}
    self.topIcons.id = iconId
    self.topIcons.duration = duration
    self.topIcons.priority = priority
  elseif priority <= self.topIcons.priority then
    self.topIcons.id = iconId
    self.topIcons.duration = duration
    self.topIcons.priority = priority
  end
end
def.method("number").SetDefaultTitleIcon = function(self, iconId)
  self.statusTitleIcon = iconId
  self:ShowTitleIcon(self.statusTitleIcon, -1, FightModel.Title_Priority.STATUS)
end
def.method("number").SetTitleIcon = function(self, iconId)
  self:SetTitleIconWithPriority(iconId, self.titlePriority)
end
def.method("number", "number").SetTitleIconWithPriority = function(self, iconId, priority)
  ...
  if iconId == 0 then
    if self.topIcons then
      local data = self.topIcons
      self.topIcons = nil
      iconId = data.id
      if 0 >= data.duration then
        data.duration = FightModel.Label_Time_Type.PERMANENT
      end
      self.labelTime = data.duration
      self.titlePriority = data.priority
    elseif self.statusTitleIcon == 0 then
      self.labelTime = FightModel.Label_Time_Type.TIMEOUT
      self.titlePriority = FightModel.Title_Priority.NONE
      self.curTitleIcon = 0
      icon:SetActive(false)
      return
    else
      iconId = self.statusTitleIcon
      self.labelTime = FightModel.Label_Time_Type.PERMANENT
      self.titlePriority = FightModel.Title_Priority.STATUS
    end
  end
  if priority > self.titlePriority then
    return
  ...
  self.titlePriority = priority
  self.curTitleIcon = iconId
  GameUtil.AsyncLoad(bundlePath, function(tex)
    ...
    if tex then
      if uiTexture and not uiTexture:get_isnil() then
        ...
        uiTexture.width = tex.width / 2
        uiTexture.height = tex.height / 2
        icon:SetActive(true)
      end
    else
...
def.method("number").RemoveTitleIcon = function(self, iconId)
  if self.curTitleIcon == iconId then
    self:SetTitleIconWithPriority(0, FightModel.Title_Priority.NONE)
  end
end
def.override("number").Update = function(self, ticks)
  ...
  if self.labelTime <= 0 then
    self.labelTime = FightModel.Label_Time_Type.TIMEOUT
    self:SetTitleIconWithPriority(0, FightModel.Title_Priority.STATUS)
  end
end
...
def.override("string", "number", "=>", "userdata").AddEffect = function(self, effectPath, part)
  local pos
  if self.m_model then
...
def.override("varlist", "=>", "userdata").AddChildEffect = function(self, ...)
  if self.m_model == nil then
    return nil
  end
  local effectPath, part, boneName, rotationOffset = select(1, ...)
  ...
  self.childEffects[effectPath] = fx
  return fx
end
def.override("string", "number", "=>", "userdata").AddEffectWithOffset = function(self, effectPath, offset)
...
def.override("string", "number", "=>", "userdata").AddEffectWithRotation = function(self, effectPath, part)
  local pos
  local model = self.m_model
  if model == nil then
    return nil
...
def.override().SetPate = function(self)
  if self.m_topIcon then
    ...
    if self.topIcons then
      local iconInfo = self.topIcons
      self.topIcons = nil
      self:ShowTitleIcon(iconInfo.id, iconInfo.duration, iconInfo.priority)
    end
  end
end
...
def.method("table").SetModelOffset = function(self, offset)
  self.model_offset = offset
  if self.m_model then
    self.m_model.localPosition = self.m_model.localPosition + offset
  end
end
def.override().TurnToStone = function(self)
  EnableLightEffect(self, false)
  if self.mECPartComponent then
    self.mECPartComponent:TurnToStone()
  end
  if self.mECWingComponent then
    self.mECWingComponent:TurnToStone()
  end
  if self.mECFabaoComponent then
    self.mECFabaoComponent:TurnToStone()
  end
  ECRoleModel.TurnToStone(self)
end
def.override().RecoverFromStone = function(self)
  if self.mECPartComponent then
    self.mECPartComponent:RecoverFromStone()
  end
  if self.mECWingComponent then
    self.mECWingComponent:RecoverFromStone()
  end
  if self.mECFabaoComponent then
    self.mECFabaoComponent:RecoverFromStone()
  end
  ECRoleModel.RecoverFromStone(self)
  EnableLightEffect(self, true)
end
def.method("number").ChangeScale = function(self, s)
  if s <= 0 then
    return
  end
  if self.scaleInfo then
    if self.scaleInfo.target_scale ~= s then
      self.scaleInfo.time = 0
      self.scaleInfo.init_scale = self.model_scale
      self.scaleInfo.range = self.model_scale - s
      self.scaleInfo.target_scale = s
    end
  elseif self.model_scale ~= s then
    self.scaleInfo = {}
    self.scaleInfo.time = 0
    self.scaleInfo.init_scale = self.model_scale
    self.scaleInfo.range = s - self.model_scale
    self.scaleInfo.target_scale = s
  end
end
def.method("number").UpdateScale = function(self, tick)
  if self.scaleInfo == nil then
    return
  end
  self.scaleInfo.time = self.scaleInfo.time + tick
  if _G.IsNil(self.m_model) then
    return
  end
  if self.scaleInfo.time < FightModel.CHANGE_SCALE_DURATION then
    self.model_scale = self.scaleInfo.init_scale + self.scaleInfo.range * (self.scaleInfo.time / FightModel.CHANGE_SCALE_DURATION)
    self.m_model.localScale = Model_Default_Scale * self.model_scale
  else
    self.model_scale = self.scaleInfo.target_scale
    self.m_model.localScale = Model_Default_Scale * self.model_scale
    self.scaleInfo = nil
  end
end
def.method("number", "number").SetModelActionMap = function(self, modelId, state)
  self.modelActionMap = require("Main.Fight.FightUtils").GetModelActionCfg(modelId, state)
end
def.method("string", "=>", "string").GetModelAction = function(self, actionName)
  if self.modelActionMap == nil then
    return actionName
  end
  local newActionName = self.modelActionMap[actionName]
  if newActionName == nil or newActionName == "" then
    newActionName = actionName
  end
  return newActionName
end
def.override("string", "=>", "number", "string").GetAniDuration = function(self, aniname)
  return ECModel.GetAniDuration(self, self:GetModelAction(aniname))
end
FightModel.Commit()
return FightModel
