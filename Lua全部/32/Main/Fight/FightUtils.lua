...
def.static("number", "number", "=>", "table").GetSkillActionCfg = function(modelId, actionId)
  ...
  if actionCfg == nil or actionId == 0 then
    return nil
  end
  local key = bit.lshift(modelId, 8) + actionId
...
def.static().LoadEffectPlayCfg = function()
  ...
  for i = 0, size - 1 do
    ...
    cfg.effectDirection = record:GetIntValue("effectDirection")
    cfg.behitInfos = {}
    local hitTimeStruct = record:GetStructValue("behitStruct")
    local count = hitTimeStruct:GetVectorSize("behitVector")
    local i = 1
...
def.static("number", "=>", "table").GetEffectGroupCfg = function(id)
  ...
  cfg.modelScale = record:GetIntValue("modelScalingPercentage") / 100
  return cfg
end
...
def.static("number", "=>", "table").GetFlyEffectCfg = function(id)
  local record = DynamicData.GetRecord(CFG_PATH.FLY_EFFECT_CFG, id)
  if record == nil then
    warn("[GetFlyEffectCfg] get nil record for id: ", id)
    return nil
  end
  local cfg = {}
  cfg.effectId = id
  cfg.start_offset = record:GetIntValue("start_offset") / 1000
  cfg.beginTime = record:GetIntValue("beginTime") / 1000
  cfg.duration = record:GetIntValue("duration") / 1000
  cfg.end_offset = record:GetIntValue("end_offset") / 1000
  return cfg
end
...
def.static("number", "=>", "table").GetFightTypeCfg = function(id)
  ...
  cfg.useFightChannel = record:GetCharValue("useFightChannel") ~= 0
  cfg.displayOperateInfoInSystemChannel = record:GetCharValue("displayOperateInfoInSystemChannel") ~= 0
  cfg.musics = {}
  cfg.bgIds = {}
...
def.static("number", "=>", "table").GetWorldBossCfg = function(id)
  local cfg = {}
  cfg.fightType = "8"
  cfg.mainMonsterModelId = "700301050"
  cfg.lightMap = "700301253"
  return cfg
end
...
def.static("userdata", "table", "=>", "table").ShowPetTeam = function(modelsRoot, petTeamInfo)
  if _G.IsNil(modelsRoot) or petTeamInfo == nil then
    return nil
  end
  local group = {}
  group.cells = {}
  for i = 1, require("Main.Pet.PetsArena.PetsArenaMgr").MAX_MODEL do
    local cell = modelsRoot:FindDirect("Group_Site_" .. i)
    table.insert(group.cells, cell)
    local Img_Site = cell:FindDirect("Img_Site")
    Img_Site:SetActive(false)
  end
  return require("Main.Fight.ui.DlgPetGroupArena").Instance():ShowPetTeam(group, petTeamInfo)
end
def.static("number", "=>", "table").GetPVEPetWheelFightCfg = function(id)
  local record = DynamicData.GetRecord(CFG_PATH.PET_WHEEL_FIGHT_CFG, id)
  if record == nil then
    warn("[GetPVEPetWheelFightCfg] get nil record for id: ", id)
    return nil
  end
  local cfg = {}
  cfg.id = record:GetIntValue("id")
  cfg.maxUseItemCount = record:GetIntValue("maxUseItemCount")
  cfg.minWatchLengthInSecond = record:GetIntValue("minWatchLengthInSecond")
  cfg.minWatchRoundNum = record:GetIntValue("minWatchRoundNum")
  cfg.wheels = {}
  local wheelStruct = record:GetStructValue("wheelStruct")
  local count = wheelStruct:GetVectorSize("wheelList")
  local i = 1
  for i = 1, count do
    local rec = wheelStruct:GetVectorValueByIdx("wheelList", i - 1)
    local info = {}
    info.index = rec:GetIntValue("wheelIndex")
    info.fightPetCfgid = rec:GetIntValue("fightPetCfgid")
    info.teamName = rec:GetStringValue("teamName")
    info.wheelMinWatchLengthInSecond = rec:GetIntValue("wheelMinWatchLengthInSecond")
    info.wheelMinWatchRoundNum = rec:GetIntValue("wheelMinWatchRoundNum")
    cfg.wheels[info.index] = info
  end
  return cfg
end
def.static("userdata", "table", "number", "number", "function").MoveObjectToTargetPos = function(obj, dest, duration, tolerance, cb)
  local dir = dest - obj.localPosition
  local dist = dir:get_Length()
  dir:Normalize()
  obj.forward = dir
  ...
  local function OnMove()
    ...
    obj.localPosition = obj.localPosition + obj.forward * step
    local curDist = dest - obj.localPosition
    if curDist:get_Length() <= tolerance or 0 > curDist:Dot(obj.forward) then
      if timerId > 0 then
        GameUtil.RemoveGlobalTimer(timerId)
        timerId = 0
      end
  ...
  timerId = GameUtil.AddGlobalTimer(0, false, OnMove)
end
def.static("number", "=>", "boolean").IsCustomFightType = function(id)
  local record = DynamicData.GetRecord(CFG_PATH.CUSTOM_FIGHT_TYPE_CFG, id)
  return record ~= nil
end
def.static("number", "=>", "number").GetSkillReturnTypeOnException = function(skillId)
  local record = DynamicData.GetRecord(CFG_PATH.SKILL_RETURN_CFG, skillId)
  if record == nil then
    return 0
  end
  return record:GetIntValue("returnType")
end
def.static("number", "number", "=>", "table").GetModelActionCfg = function(modelId, state)
  local entries = DynamicData.GetTable(CFG_PATH.MODEL_ACTION_CFG)
  if entries == nil then
    return nil
  end
  local size = DynamicDataTable.GetRecordsCount(entries)
  ...
  for i = 0, size - 1 do
    ...
    local _modelId = record:GetIntValue("modelCfgId")
    if _modelId == modelId then
      local _state = record:GetIntValue("state")
      if _state == state then
        local stateAction = {}
        stateAction[ActionName.FightStand] = record:GetStringValue("stand")
        stateAction[ActionName.BeHit] = record:GetStringValue("hit")
        stateAction[ActionName.Death1] = record:GetStringValue("dead")
        stateAction[ActionName.Attack1] = record:GetStringValue("attack")
        stateAction[ActionName.Magic] = record:GetStringValue("magic")
        stateAction[ActionName.Defend] = record:GetStringValue("defense")
        stateAction.changeState = record:GetStringValue("changeState")
        return stateAction
      end
    end
  end
