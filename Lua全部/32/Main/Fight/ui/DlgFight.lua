...
def.method("number").SetBigRound = function(self, round)
  self.big_round = round
  if self == nil or self.m_panel == nil then
    return
  end
  local big_round_panel = self.m_panel:FindDirect("Group_Battle")
  if self.big_round == 0 then
    big_round_panel:SetActive(false)
  else
    big_round_panel:SetActive(true)
    local roundLabel = dlg.m_panel:FindDirect("Group_Battle/Label_BattleNum")
    roundLabel:GetComponent("UILabel").text = tostring(self.big_round)
  end
end
def.method("number").ShowBigRoundEffect = function(self, round)
  local roundLabel = self.m_panel:FindDirect("Group_Battle/Label_BattleNum")
  local effectRes = GetEffectRes(constant.CHulaCfgConsts.POINT_EFFECTID_4)
  if effectRes then
    require("Fx.GUIFxMan").Instance():PlayAsChildLayerWithCallback(roundLabel, effectRes.path, "big_round", 0, 0, 1, 1, -1, false, function(fx, l)
      if self.m_panel == nil then
        return
      end
      ...
      if lbl then
        ...
        cmp:set_depth(l)
        cmp:set_text(tostring(round))
      end
      self:SetBigRound(round)
    end)
  end
end
