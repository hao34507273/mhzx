...
def.method("=>", "boolean").IsAnyThingAboutFormationInBag = function(self)
  ...
  local key, item = ItemModule.Instance():SelectOneItemByItemType(ItemModule.FRAGMENT_BAG, ItemType.ZHENFA_FRAGMENT_ITEM)
  if key >= 0 then
    return true
  end
...
def.method("number", "number", "=>", "number").GetFormationRelation = function(self, formationId1, formationId2)
  if formationId1 == nil then
    return 0
  end
  if formationId2 == nil then
    return 0
  end
  local formationCfg = FormationUtils.GetFormationCfg(formationId1)
  if formationCfg == nil then
    return 0
  end
  if formationCfg.KZInfo[formationId2] ~= nil then
    return 1
  elseif formationCfg.BKInfo[formationId2] ~= nil then
    return -1
  else
    return 0
  end
