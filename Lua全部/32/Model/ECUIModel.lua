    ...
    if self.mount.m_status == ModelStatus.DESTROY or self.mount.m_status == ModelStatus.NONE then
      self.mount:LoadRide(function()
        if self.mount and self.mount.m_model and not self.mount.m_model.isnil then
          ...
          if not _G.IsNil(self.m_uiModel) then
            self.mount.m_model.transform.position = EC.Vector3.new(0, 0, -100)
            self.m_uiModel.modelGameObject = self.mount.m_model
          end
        end
        if callback then
          _G.SafeCall(callback, self.mount)
        end
      end)
...
def.method("number", "number", "number", "function").TrySetMount = function(self, mountId, level, colorId, callback)
  self.mount = ECRide.new(self, mountId, level, colorId)
  local MountsUtils = require("Main.Mounts.MountsUtils")
  local mountCfg = MountsUtils.GetMountsDetailInfo(mountId, level, colorId)
  self.mount.actionList = mountCfg.actionList
  self:ReturnMount(nil)
  self.mount:RoleRideAction(self, self.mount:GetStandActionName(1))
  self.mount:Play(ActionName.Stand)
  self.mount:EndRun()
  if self:IsLoaded() then
    self:ReturnMount(callback)
  else
    self:AddOnLoadCallback("GetOnMount", function()
      self:ReturnMount(callback)
    end)
  end
end
...
def.override().OnLoadGameObject = function(self)
  ...
  if not _G.IsNil(self.m_uiModel) then
    self.m_uiModel.modelGameObject = self.m_model
  end
