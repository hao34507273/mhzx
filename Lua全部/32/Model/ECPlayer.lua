...
local EFFECT_OFFSET_POS = EC.Vector3.new(0, 0, 0)
def.method().ResetTopIcons = function(self)
  ...
  local addTitleHeight, addonHeight = self:ReposAttachedEffectsHeightInfo(FlagPosList[2].y)
  EFFECT_OFFSET_POS.y = addonHeight
  local addonModelHeight = addonHeight * 0.0159
  if addTitleHeight then
    teamlabel.localPosition = FlagPosList[2] + TEAM_LABEL_OFFSET + EFFECT_OFFSET_POS
  else
    teamlabel.localPosition = FlagPosList[3] + TEAM_LABEL_OFFSET + EFFECT_OFFSET_POS
  end
  if not _G.IsNil(self.teamIcon) then
    ...
    if self.mount and self.mount:IsObjLoaded() and isSit then
      h = h - ECRoleModel.MOUNT_TOPICON_OFFSET
    end
    ICON_POS.y = h + 0.4 + addonModelHeight
    self.teamIcon.localPosition = ICON_POS
  end
  if not _G.IsNil(self.battleIcon) then
    local box = self.m_model and self.m_model:GetComponent("BoxCollider")
    local h = box and box.size.y or 1.6
    local offset = h + 0.2
    if not _G.IsNil(self.teamIcon) then
      offset = h + 1.4
    elseif addTitleHeight then
      offset = h + 0.7
    end
    if self.mount and self.mount:IsObjLoaded() and isSit then
      offset = offset - ECRoleModel.MOUNT_TOPICON_OFFSET
    end
    ICON_POS.y = offset + addonModelHeight
    self.battleIcon.localPosition = ICON_POS
  end
end
def.method("number", "=>", "boolean", "number").ReposAttachedEffectsHeightInfo = function(self, titleHeight)
  local addTitleHeight = self.titleIcon > 0 and true or false
  local addonEffects = {}
  if self.m_attachedEffects then
    for _, v in pairs(self.m_attachedEffects) do
      local eff = v.eff
      if not _G.IsNil(eff) and eff:get_activeSelf() then
        if v.isTitle then
          addTitleHeight = true
        elseif v.height then
          table.insert(addonEffects, v)
        end
      end
    end
  end
  table.sort(addonEffects, function(lhs, rhs)
    return lhs.priority < rhs.priority
  end)
  local titleOffset = addTitleHeight and titleHeight or 0
  local addonHeight = 0
  for i, v in ipairs(addonEffects) do
    EFFECT_OFFSET_POS.y = addonHeight + titleOffset
    v.eff.localPosition = EFFECT_OFFSET_POS
    addonHeight = addonHeight + v.height
  end
  return addTitleHeight, addonHeight
end
